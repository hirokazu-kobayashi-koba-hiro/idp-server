...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X GET \
  "${BASE_URL}/v1/management/tenants/${TENANT_ID}/authentication-policies/${CONFIG_ID}" \
...
if [ "$HTTP_CODE" == "200" ]; then
  ...
  METHOD="PUT"
  URL="${BASE_URL}/v1/management/tenants/${TENANT_ID}/authentication-policies/${CONFIG_ID}${DRY_RUN_PARM}"
elif [ "$HTTP_CODE" == "404" ]; then
  echo "🆕 Config not found. Registering new one..."
  METHOD="POST"
  URL="${BASE_URL}/v1/management/tenants/${TENANT_ID}/authentication-policies${DRY_RUN_PARM}"
else
