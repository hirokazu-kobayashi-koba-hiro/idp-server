...
# =========================
usage() {
  echo "Usage: $0 -u <username> -p <password> -t <tenant_id> -f <client_json_file> [-e <base_url>] [-c <client_id>] [-s <client_secret>]"
  echo
  echo "Arguments:"
...
# 🚨 Required check
# =========================
if [ -z "$USERNAME" ] || [ -z "$PASSWORD" ] || [ -z "$TENANT_ID" ] || [ -z "$CLIENT_JSON_FILE" ]; then
  usage
fi

if [ ! -f "$CLIENT_JSON_FILE" ]; then
  echo "❌ Client config file not found: $CLIENT_JSON_FILE"
  exit 1
fi
...
# 📦 Client registration
# =========================
echo "🚀 Registering client using file: $CLIENT_JSON_FILE"

RESPONSE=$(curl -s -w "\nHTTP_CODE:%{http_code}" -X POST "${BASE_URL}/v1/management/tenants/${TENANT_ID}/clients" \
