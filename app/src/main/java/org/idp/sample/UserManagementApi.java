...
public class UserManagementApi {
    ...
    public ResponseEntity<?> get(@PathVariable("tenant-id") String tenantId,
        ...
        List<User> userList = userService.find(tenant, Integer.parseInt(limitValue), Integer.parseInt(offsetValue));
        return new ResponseEntity<>(new UserListResponse(userList), HttpStatus.OK);
    }

    @GetMapping("/{user-id}")
    public ResponseEntity<?> getById(@PathVariable("tenant-id") String tenantId,
