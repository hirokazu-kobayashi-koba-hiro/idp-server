#!/bin/bash
#
# External Password Auth - Experiment Helpers
#
# 各実験で使う共通関数と変数を定義する。
#
# 使い方:
...
#
# 使用例:
#   update_auth_config '.interactions["password-authentication"].execution.http_request.url = "http://new-service:4000/auth"'
# ============================================================

update_auth_config() {
  local jq_filter="$1"
  local auth_config_json updated response http_code
  auth_config_json=$(cat "${CONFIG_DIR}/authentication-config-password.json")
...
restore_auth_config() {
  ...
  response=$(curl -s -w "\n%{http_code}" -X PUT \
    ...
    -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d @"${CONFIG_DIR}/authentication-config-password.json")
