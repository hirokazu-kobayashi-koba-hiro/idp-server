...
# Third Party Integration - Use Case Setup Script
#
# Prerequisites:
...
echo "=========================================="
echo "Third Party Integration Use Case Setup"
echo "=========================================="
echo ""
...
# Token strategy (third-party specific)
ACCESS_TOKEN_DURATION="${ACCESS_TOKEN_DURATION:-1800}"
ID_TOKEN_DURATION="${ID_TOKEN_DURATION:-3600}"
REFRESH_TOKEN_DURATION="${REFRESH_TOKEN_DURATION:-604800}"
ROTATE_REFRESH_TOKEN="${ROTATE_REFRESH_TOKEN:-true}"
REFRESH_TOKEN_STRATEGY="${REFRESH_TOKEN_STRATEGY:-FIXED}"
...
# --- Step 4: Create public tenant (token strategy + custom scopes) ---
echo "Step 4: Creating public tenant (token strategy + custom scopes)..."
...
PUBLIC_TENANT_JSON=$(echo "${PUBLIC_TENANT_JSON}" | jq \
  ...
  --argjson rotate_rt "${ROTATE_REFRESH_TOKEN}" \
  --arg rt_strategy "${REFRESH_TOKEN_STRATEGY}" \
  '
  ...
  .authorization_server.extension.rotate_refresh_token = $rotate_rt |
  .authorization_server.extension.refresh_token_strategy = $rt_strategy
  ')
...
WEB_CLIENT_JSON=$(substitute_template "${SCRIPT_DIR}/web-client-template.json" \
  "CLIENT_ID" "${CLIENT_ID}" \
...
M2M_CLIENT_SECRET="${M2M_CLIENT_SECRET:-$(uuidgen | tr '[:upper:]' '[:lower:]')}"
M2M_CLIENT_NAME="${M2M_CLIENT_NAME:-M2M Service}"
M2M_SCOPE="${M2M_SCOPE:-api:read api:write}"

M2M_CLIENT_JSON=$(substitute_template "${SCRIPT_DIR}/m2m-client-template.json" \
  "M2M_CLIENT_ID" "${M2M_CLIENT_ID}" \
  "M2M_CLIENT_ALIAS" "${M2M_CLIENT_ALIAS}" \
  "M2M_CLIENT_SECRET" "${M2M_CLIENT_SECRET}" \
  "M2M_CLIENT_NAME" "${M2M_CLIENT_NAME}" \
  "M2M_SCOPE" "${M2M_SCOPE}")

echo "${M2M_CLIENT_JSON}" | jq '.' > "${OUTPUT_DIR}/m2m-client.json"
echo "  Saved: ${OUTPUT_DIR}/m2m-client.json"

M2M_CLIENT_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/clients" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${OUTPUT_DIR}/m2m-client.json")

HTTP_CODE=$(echo "${M2M_CLIENT_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${M2M_CLIENT_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  M2M client created: ${M2M_CLIENT_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
...
echo "  Token strategy:   AT=${ACCESS_TOKEN_DURATION}s, IDT=${ID_TOKEN_DURATION}s, RT=${REFRESH_TOKEN_DURATION}s"
echo "    Rotation: ${ROTATE_REFRESH_TOKEN}, Strategy: ${REFRESH_TOKEN_STRATEGY}"
...
echo "  ${AUTHORIZATION_SERVER_URL}/${PUBLIC_TENANT_ID}/v1/authorizations?response_type=code&client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=openid%20profile%20email%20api%3Aread&state=test-state"
echo ""
