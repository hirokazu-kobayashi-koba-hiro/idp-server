...
# Login (Social) - Use Case Setup Script
#
# Prerequisites:
...
echo "=========================================="
echo "Login (Social) Use Case Setup"
echo "=========================================="
echo ""
...
FEDERATION_ID="${FEDERATION_ID:-$(uuidgen | tr '[:upper:]' '[:lower:]')}"
...
echo "${FEDERATION_JSON}" | jq '.' > "${OUTPUT_DIR}/federation-config.json"
echo "  Saved: ${OUTPUT_DIR}/federation-config.json"

FEDERATION_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/federation-configurations" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${OUTPUT_DIR}/federation-config.json")

HTTP_CODE=$(echo "${FEDERATION_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${FEDERATION_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  Federation configuration created: ${FEDERATION_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
...
# --- Step 9: Update client with federation settings ---
echo "Step 9: Updating client with federation settings..."

CLIENT_WITH_FEDERATION=$(jq --arg fed_id "${FEDERATION_ID}" '. + {"available_federations": [$fed_id]}' "${OUTPUT_DIR}/public-client.json")
echo "${CLIENT_WITH_FEDERATION}" | jq '.' > "${OUTPUT_DIR}/public-client.json"
echo "  Updated: ${OUTPUT_DIR}/public-client.json"
...
echo "  Auth policy:      password only (social login via federation)"
echo "  Federation:       Google OIDC (${GOOGLE_CLIENT_ID})"
echo ""
...
echo "  Federation:    Google (${FEDERATION_ID})"
echo ""
...
echo "  ${OUTPUT_DIR}/federation-config.json"
echo ""
...
echo ""
echo "  2. Register or login with email/password, or click 'Sign in with Google'"
echo ""
