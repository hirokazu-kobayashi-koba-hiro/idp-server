#!/bin/bash
#
# External API Auth - Experiment Helpers
#
# 各実験で使う共通関数と変数を定義する。
#
# 使い方:
...
AUTH_CONFIG_ID=$(jq -r '.id' "${CONFIG_DIR}/authentication-config-external-api.json")
...
#
# 使用例:
#   update_auth_config '.interactions.password_verify.execution.http_request.url = "http://new-service:4000/auth"'
# ============================================================

update_auth_config() {
  local jq_filter="$1"
  local auth_config_json updated response http_code
  auth_config_json=$(cat "${CONFIG_DIR}/authentication-config-external-api.json")
  updated=$(echo "${auth_config_json}" | jq "${jq_filter}")
  ...
  if [ "${http_code}" != "200" ]; then
    echo "Error: update_auth_config failed (HTTP ${http_code})" >&2
    echo "${body}" >&2
    return 1
  fi
...
restore_auth_config() {
  ...
  response=$(curl -s -w "\n%{http_code}" -X PUT \
    ...
    -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d @"${CONFIG_DIR}/authentication-config-external-api.json")

  http_code=$(echo "${response}" | tail -1)

  if [ "${http_code}" != "200" ]; then
    echo "Error: restore_auth_config failed (HTTP ${http_code})" >&2
    echo "${response}" | sed '$d' >&2
    return 1
  fi

  echo "Authentication configuration restored."
}
...
external_api_login() {
  ...
  response=$(curl -s -w "\n%{http_code}" -b "${COOKIE_JAR}" -c "${COOKIE_JAR}" \
    ...
    -H "Content-Type: application/json" \
    -d "{\"interaction\": \"${interaction}\", \"username\": \"${username}\", \"password\": \"${password}\"}")

  http_code=$(echo "${response}" | tail -1)
  body=$(echo "${response}" | sed '$d')

  echo "← ${http_code} POST /external-api-authentication (interaction=${interaction})" >&2
  echo "${body}"
}
...
show_amr() {
  ...
  decode_jwt_payload "${id_token}" | jq '{sub, amr}'
}
