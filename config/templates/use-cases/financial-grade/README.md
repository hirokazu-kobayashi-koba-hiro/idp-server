...
| CIBA デバイス認証 | FIDO-UAF（生体認証）+ device_secret_jwt |
| eKYC（身元確認） | 有効（jp_aml、no_action モード） |
...
| `authentication-config-fido-uaf.json` | FIDO-UAF 認証設定（CIBA デバイス認証用） | `POST .../authentication-configurations` |
| `identity-verification-config.json` | 身元確認（eKYC）設定 | `POST .../identity-verification-configurations` |
...
| `MTLS_BASE_URL` | `https://mtls.api.local.test` | mTLS エンドポイント URL |
...
| 環境変数 | デフォルト | 説明 |
|---------|-----------|------|
| `FIDO_UAF_SERVER_URL` | `http://host.docker.internal:4000` | FIDO-UAF サーバー URL |

#### FIDO2 設定

| 環境変数 | デフォルト | 説明 |
|---------|-----------|------|
| `FIDO2_RP_ID` | `local.test` | WebAuthn RP ID |
...
Step 5d: 認証設定作成 - FIDO-UAF（組織レベルAPI）
  +-- POST .../tenants/{id}/authentication-configurations
     -> FIDO-UAF 認証（CIBAデバイス認証用）

Step 5e: 身元確認設定作成 - eKYC（組織レベルAPI）
  +-- POST .../tenants/{id}/identity-verification-configurations
     -> 犯収法準拠の本人確認設定（jp_aml）
