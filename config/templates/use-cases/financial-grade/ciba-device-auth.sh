...
# Simulates device-side authentication for CIBA flow using
# device_secret_jwt + FIDO-UAF authentication.
#
# Prerequisites:
#   - setup.sh has been run
#   - verify.sh has been run (creates device-credentials.json with device_id, device_secret)
#   - Mockoon FIDO-UAF mock server is running (or FIDO_UAF_SERVER_URL configured)
#
# Usage:
...
# Load device credentials (created by verify.sh or setup with device registration)
...
TENANT_ID=$(jq -r '.tenant.id' "${CONFIG_DIR}/financial-tenant.json")
