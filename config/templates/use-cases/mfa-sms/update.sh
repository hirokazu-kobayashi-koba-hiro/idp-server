...
# MFA (Password + SMS OTP) - Use Case Update Script
...
echo "=========================================="
echo "MFA (Password + SMS OTP) Use Case Update"
echo "=========================================="
...
SMS_AUTH_CONFIG_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-config-sms.json")
...
echo "   SMS Auth Config ID:   ${SMS_AUTH_CONFIG_ID}"
...
# Step 6: Update authentication configuration (sms)
echo "Step 6: Updating authentication configuration (sms)..."
SMS_AUTH_CONFIG_JSON=$(cat "${OUTPUT_DIR}/authentication-config-sms.json")

SMS_AUTH_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations/${SMS_AUTH_CONFIG_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${SMS_AUTH_CONFIG_JSON}")

SMS_AUTH_CONFIG_HTTP_CODE=$(echo "${SMS_AUTH_CONFIG_RESPONSE}" | tail -n1)
SMS_AUTH_CONFIG_RESPONSE_BODY=$(echo "${SMS_AUTH_CONFIG_RESPONSE}" | sed '$d')

if [ "${SMS_AUTH_CONFIG_HTTP_CODE}" = "200" ] || [ "${SMS_AUTH_CONFIG_HTTP_CODE}" = "204" ]; then
  echo "Authentication configuration (sms) updated successfully"
else
  echo "Warning: SMS authentication configuration update failed (HTTP ${SMS_AUTH_CONFIG_HTTP_CODE})"
  echo "Response: ${SMS_AUTH_CONFIG_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${SMS_AUTH_CONFIG_RESPONSE_BODY}"
fi
echo ""
