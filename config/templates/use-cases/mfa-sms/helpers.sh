#!/bin/bash
#
# MFA (Password + SMS OTP) - Experiment Helpers
#
...
#   update_sms_config            SMS OTP設定更新（jqフィルタ）
#   restore_sms_config           SMS OTP設定を初期状態に復元
#   get_sms_config               SMS OTP設定取得
...
#   sms_challenge                SMS OTP チャレンジ送信
#   get_verification_code        Management API で検証コード取得
#   sms_verify                   SMS OTP 検証
#   password_login               パスワード認証
#   complete_auth_flow           認可→コード取得→トークン交換
#   mfa_login                    MFA ログイン一括実行（password → sms challenge → verify → authorize → token）
#
#   === ユーティリティ ===
...
SMS_AUTH_CONFIG_ID=$(jq -r '.id' "${CONFIG_DIR}/authentication-config-sms.json")
SMS_AUTH_CONFIG_JSON_INITIAL=$(cat "${CONFIG_DIR}/authentication-config-sms.json")
SMS_AUTH_CONFIG_JSON="${SMS_AUTH_CONFIG_JSON_INITIAL}"
...
echo "=========================================="
echo "MFA SMS Experiment Helpers Loaded"
echo "=========================================="
...
echo "  SMS Config ID:       ${SMS_AUTH_CONFIG_ID}"
...
#
# 使用例:
#   update_sms_config '.interactions["sms-authentication-challenge"].execution.details.expire_seconds = 10'
# ============================================================
...
update_sms_config() {
  ...
  local tmpfile
  tmpfile=$(mktemp)
  printf '%s\n' "${SMS_AUTH_CONFIG_JSON}" | jq "${jq_filter}" > "${tmpfile}"
  updated=$(cat "${tmpfile}")

  response=$(curl -s -w "\n%{http_code}" -X PUT \
    "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations/${SMS_AUTH_CONFIG_ID}" \
    -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d @"${tmpfile}")
  ...
  if [ "${http_code}" != "200" ]; then
    echo "Error: update_sms_config failed (HTTP ${http_code})" >&2
    echo "${body}" >&2
    return 1
  fi

  SMS_AUTH_CONFIG_JSON="${updated}"
  echo "${body}"
}
...
restore_sms_config() {
  ...
  local tmpfile
  tmpfile=$(mktemp)
  printf '%s\n' "${SMS_AUTH_CONFIG_JSON_INITIAL}" > "${tmpfile}"
  ...
  if [ "${http_code}" != "200" ]; then
    echo "Error: restore_sms_config failed (HTTP ${http_code})" >&2
    echo "${response}" | sed '$d' >&2
    return 1
  fi

  SMS_AUTH_CONFIG_JSON="${SMS_AUTH_CONFIG_JSON_INITIAL}"
  echo "SMS config restored."
}

get_sms_config() {
  curl -s "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations/${SMS_AUTH_CONFIG_ID}" \
    -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}"
}
...
sms_challenge() {
  ...
  local response http_code body
  response=$(curl -s -w "\n%{http_code}" -b "${COOKIE_JAR}" -c "${COOKIE_JAR}" \
    -X POST "${TENANT_BASE}/v1/authorizations/${AUTHORIZATION_ID}/sms-authentication-challenge" \
    -H "Content-Type: application/json" \
    -d "{\"phone_number\": \"${phone_number}\", \"template\": \"authentication\"}")

  http_code=$(echo "${response}" | tail -1)
  body=$(echo "${response}" | sed '$d')

  if [ "${http_code}" = "200" ]; then
    echo "← ${http_code} Challenge sent to ${phone_number}" >&2
  else
...
sms_verify() {
  ...
  local response http_code body
  response=$(curl -s -w "\n%{http_code}" -b "${COOKIE_JAR}" -c "${COOKIE_JAR}" \
    -X POST "${TENANT_BASE}/v1/authorizations/${AUTHORIZATION_ID}/sms-authentication" \
    -H "Content-Type: application/json" \
  ...
  if [ "${http_code}" = "200" ]; then
    echo "← ${http_code} SMS verified successfully" >&2
  elif [ "${http_code}" = "400" ]; then
    echo "← ${http_code} Verification failed (code expired or invalid)" >&2
  else
    echo "← ${http_code} SMS verification error" >&2
  fi
  echo "${body}"
}
...
# MFA ログイン一括実行
# 使用例:
#   mfa_login "${TEST_PHONE}" "${TEST_PASSWORD}"
mfa_login() {
  local phone="$1"
  local password="$2"

  start_auth_flow
  password_login "${phone}" "${password}"
  sms_challenge "${phone}"
  get_verification_code
  sms_verify
  complete_auth_flow
