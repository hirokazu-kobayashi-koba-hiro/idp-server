...
# MFA (Password + SMS OTP) - Use Case Setup Script
#
# Prerequisites:
...
echo "=========================================="
echo "MFA (Password + SMS OTP) Use Case Setup"
echo "=========================================="
echo ""
...
# --- Step 6: Create authentication configuration (sms) ---
echo "Step 6: Creating authentication configuration (sms)..."
...
SMS_AUTH_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  ...
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${OUTPUT_DIR}/authentication-config-sms.json")

HTTP_CODE=$(echo "${SMS_AUTH_CONFIG_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${SMS_AUTH_CONFIG_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  SMS authentication configuration created: ${SMS_AUTH_CONFIG_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
...
# --- Step 7: Create authentication policy (password AND SMS MFA) ---
echo "Step 7: Creating authentication policy (password + SMS MFA)..."
...
echo "  ${OUTPUT_DIR}/authentication-config-sms.json"
