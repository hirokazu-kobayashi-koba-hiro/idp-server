...
# CIBA (Client-Initiated Backchannel Authentication) - Use Case Update Script
# Updates public tenant, authorization server, client, auth configs, policies, and user.
#
# Usage:
#   ./update.sh
...
FIDO_UAF_AUTH_CONFIG_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-config-fido-uaf.json" 2>/dev/null || echo "")
AUTH_POLICY_OAUTH_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-policy-oauth.json")
AUTH_POLICY_CIBA_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-policy-ciba.json")
...
echo "   FIDO-UAF Auth Config ID: ${FIDO_UAF_AUTH_CONFIG_ID}"
echo "   Auth Policy OAuth ID:   ${AUTH_POLICY_OAUTH_ID}"
echo "   Auth Policy CIBA ID:    ${AUTH_POLICY_CIBA_ID}"
echo ""
...
# Step 6a: Update OAuth authentication policy
echo "Step 6a: Updating OAuth authentication policy..."
AUTH_POLICY_OAUTH_JSON=$(cat "${OUTPUT_DIR}/authentication-policy-oauth.json")

AUTH_POLICY_OAUTH_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-policies/${AUTH_POLICY_OAUTH_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${AUTH_POLICY_OAUTH_JSON}")

AUTH_POLICY_OAUTH_HTTP_CODE=$(echo "${AUTH_POLICY_OAUTH_RESPONSE}" | tail -n1)
AUTH_POLICY_OAUTH_RESPONSE_BODY=$(echo "${AUTH_POLICY_OAUTH_RESPONSE}" | sed '$d')

if [ "${AUTH_POLICY_OAUTH_HTTP_CODE}" = "200" ] || [ "${AUTH_POLICY_OAUTH_HTTP_CODE}" = "204" ]; then
  echo "OAuth authentication policy updated successfully"
else
  echo "Warning: OAuth authentication policy update failed (HTTP ${AUTH_POLICY_OAUTH_HTTP_CODE})"
  echo "Response: ${AUTH_POLICY_OAUTH_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${AUTH_POLICY_OAUTH_RESPONSE_BODY}"
fi
echo ""

# Step 6b: Update CIBA authentication policy
echo "Step 6b: Updating CIBA authentication policy..."
AUTH_POLICY_CIBA_JSON=$(cat "${OUTPUT_DIR}/authentication-policy-ciba.json")

AUTH_POLICY_CIBA_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-policies/${AUTH_POLICY_CIBA_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${AUTH_POLICY_CIBA_JSON}")

AUTH_POLICY_CIBA_HTTP_CODE=$(echo "${AUTH_POLICY_CIBA_RESPONSE}" | tail -n1)
AUTH_POLICY_CIBA_RESPONSE_BODY=$(echo "${AUTH_POLICY_CIBA_RESPONSE}" | sed '$d')

if [ "${AUTH_POLICY_CIBA_HTTP_CODE}" = "200" ] || [ "${AUTH_POLICY_CIBA_HTTP_CODE}" = "204" ]; then
  echo "CIBA authentication policy updated successfully"
else
  echo "Warning: CIBA authentication policy update failed (HTTP ${AUTH_POLICY_CIBA_HTTP_CODE})"
  echo "Response: ${AUTH_POLICY_CIBA_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${AUTH_POLICY_CIBA_RESPONSE_BODY}"
fi
echo ""

# Step 7: Update FIDO-UAF authentication configuration
if [ -n "${FIDO_UAF_AUTH_CONFIG_ID}" ] && [ "${FIDO_UAF_AUTH_CONFIG_ID}" != "null" ] && [ "${FIDO_UAF_AUTH_CONFIG_ID}" != "" ]; then
  echo "Step 7: Updating FIDO-UAF authentication configuration..."
  FIDO_UAF_CONFIG_JSON=$(cat "${OUTPUT_DIR}/authentication-config-fido-uaf.json")

  FIDO_UAF_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
    "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations/${FIDO_UAF_AUTH_CONFIG_ID}" \
    -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d "${FIDO_UAF_CONFIG_JSON}")

  FIDO_UAF_CONFIG_HTTP_CODE=$(echo "${FIDO_UAF_CONFIG_RESPONSE}" | tail -n1)
  FIDO_UAF_CONFIG_RESPONSE_BODY=$(echo "${FIDO_UAF_CONFIG_RESPONSE}" | sed '$d')

  if [ "${FIDO_UAF_CONFIG_HTTP_CODE}" = "200" ] || [ "${FIDO_UAF_CONFIG_HTTP_CODE}" = "204" ]; then
    echo "FIDO-UAF authentication configuration updated successfully"
  else
    echo "Warning: FIDO-UAF authentication configuration update failed (HTTP ${FIDO_UAF_CONFIG_HTTP_CODE})"
    echo "Response: ${FIDO_UAF_CONFIG_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${FIDO_UAF_CONFIG_RESPONSE_BODY}"
  fi
  echo ""
else
  echo "Step 7: Skipped (no FIDO-UAF authentication configuration found)"
  echo ""
fi
