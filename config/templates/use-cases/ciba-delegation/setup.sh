...
# CIBA (Client-Initiated Backchannel Authentication) - Use Case Setup Script
#
# Prerequisites:
...
# CIBA-specific configuration
CIBA_DELIVERY_MODE="${CIBA_DELIVERY_MODE:-poll}"
...
FIDO_UAF_AUTH_CONFIG_ID="${FIDO_UAF_AUTH_CONFIG_ID:-$(uuidgen | tr '[:upper:]' '[:lower:]')}"

# Device secret configuration
MAX_DEVICES="${MAX_DEVICES:-5}"
DEVICE_SECRET_ALGORITHM="${DEVICE_SECRET_ALGORITHM:-HS256}"
DEVICE_SECRET_EXPIRES_IN="${DEVICE_SECRET_EXPIRES_IN:-31536000}"
...
# The delegation configuration embeds the client credentials it uses against the CIBA endpoints,
# so the client identity has to be decided before the authentication configuration is written.
...
# --- Step 4: Create public tenant (with password policy + CIBA config) ---
echo "Step 4: Creating public tenant (password policy + session config + CIBA)..."
...
PUBLIC_TENANT_JSON=$(echo "${PUBLIC_TENANT_JSON}" | jq \
  ...
  --arg ds_algorithm "${DEVICE_SECRET_ALGORITHM}" \
  --argjson ds_expires "${DEVICE_SECRET_EXPIRES_IN}" \
  '
  ...
  .tenant.identity_policy_config.authentication_device_rule.device_secret_algorithm = $ds_algorithm |
  .tenant.identity_policy_config.authentication_device_rule.device_secret_expires_in_seconds = $ds_expires
  ')
...
FIDO_UAF_CONFIG_JSON=$(echo "${FIDO_UAF_CONFIG_JSON}" | jq --arg id "${FIDO_UAF_AUTH_CONFIG_ID}" '. + {id: $id}')
...
if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  FIDO-UAF authentication configuration created: ${FIDO_UAF_AUTH_CONFIG_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
...
# --- Step 5c: Create authentication configuration (CIBA delegation via external API) ---
echo "Step 5c: Creating authentication configuration (CIBA delegation)..."

EXTERNAL_API_AUTH_CONFIG_ID="${EXTERNAL_API_AUTH_CONFIG_ID:-$(uuidgen | tr '[:upper:]' '[:lower:]')}"

EXTERNAL_API_CONFIG_JSON=$(substitute_template "${SCRIPT_DIR}/authentication-config-external-api.json" \
  "BASE_URL" "${AUTHORIZATION_SERVER_URL}" \
  "TENANT_ID" "${PUBLIC_TENANT_ID}" \
  "CLIENT_ALIAS" "${CLIENT_ALIAS}" \
  "CLIENT_SECRET" "${CLIENT_SECRET}")

EXTERNAL_API_CONFIG_JSON=$(echo "${EXTERNAL_API_CONFIG_JSON}" | jq --arg id "${EXTERNAL_API_AUTH_CONFIG_ID}" '. + {id: $id}')

echo "${EXTERNAL_API_CONFIG_JSON}" | jq '.' > "${OUTPUT_DIR}/authentication-config-external-api.json"
echo "  Saved: ${OUTPUT_DIR}/authentication-config-external-api.json"

EXTERNAL_API_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${OUTPUT_DIR}/authentication-config-external-api.json")

HTTP_CODE=$(echo "${EXTERNAL_API_CONFIG_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${EXTERNAL_API_CONFIG_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  CIBA delegation authentication configuration created: ${EXTERNAL_API_AUTH_CONFIG_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
...
# --- Step 6a: Create authentication policy (OAuth - CIBA delegation) ---
echo "Step 6a: Creating authentication policy (OAuth - CIBA delegation)..."

AUTH_POLICY_OAUTH_ID="${AUTH_POLICY_OAUTH_ID:-$(uuidgen | tr '[:upper:]' '[:lower:]')}"
OAUTH_POLICY_JSON=$(substitute_template "${SCRIPT_DIR}/authentication-policy-oauth.json" \
  "CLIENT_ALIAS" "${CLIENT_ALIAS}" \
  "CLIENT_ID" "${CLIENT_ID}")
echo "${OAUTH_POLICY_JSON}" | jq --arg id "${AUTH_POLICY_OAUTH_ID}" '. + {id: $id}' > "${OUTPUT_DIR}/authentication-policy-oauth.json"
...
# --- Step 6b: Create authentication policy (CIBA - fido-uaf) ---
echo "Step 6b: Creating authentication policy (CIBA - fido-uaf)..."
...
echo "  Auth policies:    OAuth (password + fido-uaf) + CIBA (fido-uaf)"
echo "  CIBA config:      delivery=${CIBA_DELIVERY_MODE}, expires=${CIBA_REQUEST_EXPIRES_IN}s, interval=${CIBA_POLLING_INTERVAL}s"
echo "  FIDO-UAF server:  ${FIDO_UAF_SERVER_URL}"
echo "  Device secret:    algorithm=${DEVICE_SECRET_ALGORITHM}, expires=${DEVICE_SECRET_EXPIRES_IN}s, max_devices=${MAX_DEVICES}"
echo ""
...
echo ""
echo "FIDO-UAF Configuration:"
echo "  Config ID:   ${FIDO_UAF_AUTH_CONFIG_ID}"
echo "  Server URL:  ${FIDO_UAF_SERVER_URL}"
echo ""
...
echo ""
echo "Test Full Flow (User Registration + FIDO-UAF + CIBA):"
echo "  ./verify.sh"
echo ""
echo "Test CIBA Device Authentication (after verify.sh):"
echo "  ./ciba-device-auth.sh"
