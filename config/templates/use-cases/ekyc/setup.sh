...
# eKYC (Identity Verification) - Use Case Setup Script
#
# Prerequisites:
...
echo "=========================================="
echo "eKYC (Identity Verification) Use Case Setup"
echo "=========================================="
echo ""
...
# --- Step 4: Create public tenant (with password policy + verified_claims) ---
echo "Step 4: Creating public tenant (password policy + session config + verified_claims)..."
...
IDENTITY_VERIFICATION_CONFIG_ID=$(uuidgen | tr '[:upper:]' '[:lower:]')

IDENTITY_VERIFICATION_JSON=$(jq --arg id "${IDENTITY_VERIFICATION_CONFIG_ID}" '. + {id: $id}' \
  "${SCRIPT_DIR}/identity-verification-config.json")

echo "${IDENTITY_VERIFICATION_JSON}" | jq '.' > "${OUTPUT_DIR}/identity-verification-config.json"
...
RESPONSE_BODY=$(echo "${IV_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  Identity verification configuration created: ${IDENTITY_VERIFICATION_CONFIG_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
