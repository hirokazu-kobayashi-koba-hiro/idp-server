...
# eKYC (Identity Verification) - Use Case Update Script
...
echo "=========================================="
echo "eKYC (Identity Verification) Use Case Update"
echo "=========================================="
...
IV_CONFIG_ID=$(jq -r '.id' "${OUTPUT_DIR}/identity-verification-config.json")
...
# Step 7: Update identity verification configuration
echo "Step 7: Updating identity verification configuration..."
IV_CONFIG_JSON=$(cat "${OUTPUT_DIR}/identity-verification-config.json")
...
IV_CONFIG_HTTP_CODE=$(echo "${IV_CONFIG_RESPONSE}" | tail -n1)
IV_CONFIG_RESPONSE_BODY=$(echo "${IV_CONFIG_RESPONSE}" | sed '$d')

if [ "${IV_CONFIG_HTTP_CODE}" = "200" ] || [ "${IV_CONFIG_HTTP_CODE}" = "204" ]; then
  echo "Identity verification configuration updated successfully"
else
  echo "Warning: Identity verification configuration update failed (HTTP ${IV_CONFIG_HTTP_CODE})"
  echo "Response: ${IV_CONFIG_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${IV_CONFIG_RESPONSE_BODY}"
fi
echo ""
