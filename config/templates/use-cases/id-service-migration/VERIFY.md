...
## Phase 4: CIBA フロー（FIDO-UAF 認証）

Phase 2/3 ではバインディングメッセージ検証で認証を完了しましたが、Phase 4 ではFIDO-UAF認証を使ってCIBAフローを実行します。

### Step 16: Backchannel Authentication Request（FIDO-UAF用）
...
### Step 17: CIBA Token Polling（FIDO-UAF認証前）
...
### Step 19: FIDO-UAF 認証チャレンジ

認証デバイスからFIDO-UAF認証チャレンジを取得します。

#### リクエスト

```bash
fido_uaf_authentication_challenge "${TRANSACTION_ID}"
echo "Challenge: ${FIDO_CHALLENGE}"
```
...
チャレンジに対してFIDO-UAF認証レスポンスを送信します。

#### リクエスト

```bash
fido_uaf_authentication "${TRANSACTION_ID}" "${FIDO_CHALLENGE}"
```
...
### Step 21: CIBA Token + UserInfo（FIDO-UAF認証後）
...
| Step | 確認項目 | 結果 |
|------|---------|------|
| 16 | CIBA リクエストで auth_req_id が取得できる | |
| 17 | 認証前ポーリングで authorization_pending が返る | |
| 18 | 認証トランザクションが取得できる | |
| 19 | FIDO-UAF 認証チャレンジが取得できる | |
| 20 | FIDO-UAF 認証が成功する（HTTP 200） | |
| 21 | CIBA token で access_token が取得でき、sub が Phase 1 と一致する | |
