...
# MFA (Password + Email OTP) - Use Case Update Script
...
echo "=========================================="
echo "MFA (Password + Email OTP) Use Case Update"
echo "=========================================="
...
echo "   Email Auth Config ID: ${EMAIL_AUTH_CONFIG_ID}"
...
# Step 6: Update authentication configuration (email)
echo "Step 6: Updating authentication configuration (email)..."
EMAIL_AUTH_CONFIG_JSON=$(cat "${OUTPUT_DIR}/authentication-config-email.json")

EMAIL_AUTH_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations/${EMAIL_AUTH_CONFIG_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${EMAIL_AUTH_CONFIG_JSON}")

EMAIL_AUTH_CONFIG_HTTP_CODE=$(echo "${EMAIL_AUTH_CONFIG_RESPONSE}" | tail -n1)
EMAIL_AUTH_CONFIG_RESPONSE_BODY=$(echo "${EMAIL_AUTH_CONFIG_RESPONSE}" | sed '$d')

if [ "${EMAIL_AUTH_CONFIG_HTTP_CODE}" = "200" ] || [ "${EMAIL_AUTH_CONFIG_HTTP_CODE}" = "204" ]; then
  echo "Authentication configuration (email) updated successfully"
else
  echo "Warning: Email authentication configuration update failed (HTTP ${EMAIL_AUTH_CONFIG_HTTP_CODE})"
  echo "Response: ${EMAIL_AUTH_CONFIG_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${EMAIL_AUTH_CONFIG_RESPONSE_BODY}"
fi
echo ""
