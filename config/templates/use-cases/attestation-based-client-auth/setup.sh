...
ORGANIZATION_NAME="${ORGANIZATION_NAME:-attestation-based-client-auth}"
...
# --- Step 10: Generate Client Attester key pair ---
echo "Step 10: Generating Client Attester key pair (ES256)..."

# The Attester's private key never reaches the Authorization Server: only the public JWK Set is
# registered on the client. Node's built-in JWK export is used so the template needs no extra deps.
ATTESTER_KEYS=$(node -e '
const crypto = require("crypto");
const { publicKey, privateKey } = crypto.generateKeyPairSync("ec", { namedCurve: "P-256" });
const kid = "attester-1";
const pub = { ...publicKey.export({ format: "jwk" }), use: "sig", kid, alg: "ES256" };
const priv = { ...privateKey.export({ format: "jwk" }), use: "sig", kid, alg: "ES256" };
console.log(JSON.stringify({ jwks: { keys: [pub] }, private_key: priv }));
')

echo "${ATTESTER_KEYS}" | jq '.' > "${OUTPUT_DIR}/attester-keys.json"
echo "  Saved: ${OUTPUT_DIR}/attester-keys.json (contains the Attester PRIVATE key)"

ATTESTER_JWKS_CONTENT=$(echo "${ATTESTER_KEYS}" | jq -c '.jwks')
echo ""

# --- Step 11: Create ABCA client (attester_jwks) ---
echo "Step 11: Creating ABCA client (trust_source=attester_jwks)..."

ATTESTER_CLIENT_ID="${ATTESTER_CLIENT_ID:-$(uuidgen | tr '[:upper:]' '[:lower:]')}"
ATTESTER_CLIENT_ALIAS="${ATTESTER_CLIENT_ALIAS:-attested-app}"
ATTESTER_CLIENT_NAME="${ATTESTER_CLIENT_NAME:-Attested Native App}"
REDIRECT_URI="${REDIRECT_URI:-http://localhost:3000/callback/}"

# client_attestation_attester_jwks is stored as a JSON *string*, so the JWK Set is embedded escaped.
ATTESTER_CLIENT_JSON=$(substitute_template "${SCRIPT_DIR}/attester-jwks-client-template.json" \
  "ATTESTER_CLIENT_ID" "${ATTESTER_CLIENT_ID}" \
  "ATTESTER_CLIENT_ALIAS" "${ATTESTER_CLIENT_ALIAS}" \
  "ATTESTER_CLIENT_NAME" "${ATTESTER_CLIENT_NAME}" \
  "REDIRECT_URI" "${REDIRECT_URI}")

echo "${ATTESTER_CLIENT_JSON}" \
  | jq --arg jwks "${ATTESTER_JWKS_CONTENT}" '.extension.client_attestation_attester_jwks = $jwks' \
  > "${OUTPUT_DIR}/attester-jwks-client.json"

ATTESTER_CLIENT_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/clients" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${OUTPUT_DIR}/attester-jwks-client.json")

HTTP_CODE=$(echo "${ATTESTER_CLIENT_RESPONSE}" | tail -n1)
if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  Client created: ${ATTESTER_CLIENT_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "${ATTESTER_CLIENT_RESPONSE}" | sed '$d' | jq '.' 2>/dev/null
  exit 1
fi
echo ""

# --- Step 12: Create ABCA client (registered_instance_key) ---
echo "Step 12: Creating ABCA client (trust_source=registered_instance_key)..."
...
SELF_SIGNED_CLIENT_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/clients" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${OUTPUT_DIR}/self-signed-client.json")

HTTP_CODE=$(echo "${SELF_SIGNED_CLIENT_RESPONSE}" | tail -n1)
if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  Client created: ${SELF_SIGNED_CLIENT_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "${SELF_SIGNED_CLIENT_RESPONSE}" | sed '$d' | jq '.' 2>/dev/null
  exit 1
fi
echo ""
...
echo ""
echo "ABCA Client (attester_jwks):"
echo "  Client ID:     ${ATTESTER_CLIENT_ID}"
echo "  Trust source:  attester_jwks"
echo "  Attester keys: ${OUTPUT_DIR}/attester-keys.json"
echo ""
...
echo ""
echo "Challenge endpoint (Section 6.3):"
echo "  ${AUTHORIZATION_SERVER_URL}/${PUBLIC_TENANT_ID}/v1/client-attestation/challenges"
echo ""
echo "Next: ./verify.sh   (checks the published metadata and the challenge endpoint)"
echo "      See VERIFY.md for how to mint the two JWTs and call the token endpoint."
echo ""
