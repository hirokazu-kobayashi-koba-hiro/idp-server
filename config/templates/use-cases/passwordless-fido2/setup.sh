...
# Passwordless (FIDO2/WebAuthn) - Use Case Setup Script
#
# Prerequisites:
...
echo "=========================================="
echo "Passwordless (FIDO2/WebAuthn) Use Case Setup"
echo "=========================================="
echo ""
...
# --- Step 4: Create public tenant (with password policy + device rule) ---
...
PUBLIC_TENANT_JSON=$(echo "${PUBLIC_TENANT_JSON}" | jq \
  ...
  --argjson device_secret_expires "${DEVICE_SECRET_EXPIRES_IN_SECONDS}" \
  '
  ...
  .tenant.identity_policy_config.authentication_device_rule.device_secret_expires_in_seconds = $device_secret_expires
  ')
...
FIDO2_AUTH_CONFIG_JSON=$(substitute_template "${SCRIPT_DIR}/authentication-config-fido2-template.json" \
  "FIDO2_RP_ID" "${FIDO2_RP_ID}" \
  "FIDO2_RP_NAME" "${FIDO2_RP_NAME}" \
  "FIDO2_ALLOWED_ORIGIN" "${FIDO2_ALLOWED_ORIGIN}")

FIDO2_AUTH_CONFIG_JSON=$(echo "${FIDO2_AUTH_CONFIG_JSON}" | jq --arg id "${FIDO2_AUTH_CONFIG_ID}" '. + {id: $id}')

echo "${FIDO2_AUTH_CONFIG_JSON}" | jq '.' > "${OUTPUT_DIR}/authentication-config-fido2.json"
...
HTTP_CODE=$(echo "${FIDO2_AUTH_CONFIG_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${FIDO2_AUTH_CONFIG_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "  FIDO2 authentication configuration created: ${FIDO2_AUTH_CONFIG_ID}"
else
  echo "  Failed (HTTP ${HTTP_CODE})"
  echo "  ${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "  ${RESPONSE_BODY}"
  exit 1
...
echo ""
echo "  2. Register with email/password, then authenticate with FIDO2 or password"
echo ""
