...
# Passwordless (FIDO2/WebAuthn) - Use Case Update Script
...
echo "=========================================="
echo "Passwordless (FIDO2/WebAuthn) Use Case Update"
echo "=========================================="
...
FIDO2_AUTH_CONFIG_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-config-fido2.json")
AUTH_POLICY_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-policy.json")
AUTH_POLICY_FIDO2_REG_ID=$(jq -r '.id' "${OUTPUT_DIR}/authentication-policy-fido2-registration.json")
...
echo "   FIDO2 Auth Config ID: ${FIDO2_AUTH_CONFIG_ID}"
echo "   Auth Policy ID:       ${AUTH_POLICY_ID}"
echo "   FIDO2 Reg Policy ID:  ${AUTH_POLICY_FIDO2_REG_ID}"
echo ""
...
# Step 5: Update authentication configuration (initial-registration)
echo "Step 5: Updating authentication configuration (initial-registration)..."
...
# Step 6: Update authentication configuration (fido2)
echo "Step 6: Updating authentication configuration (fido2)..."
FIDO2_AUTH_CONFIG_JSON=$(cat "${OUTPUT_DIR}/authentication-config-fido2.json")

FIDO2_AUTH_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations/${FIDO2_AUTH_CONFIG_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${FIDO2_AUTH_CONFIG_JSON}")

FIDO2_AUTH_CONFIG_HTTP_CODE=$(echo "${FIDO2_AUTH_CONFIG_RESPONSE}" | tail -n1)
FIDO2_AUTH_CONFIG_RESPONSE_BODY=$(echo "${FIDO2_AUTH_CONFIG_RESPONSE}" | sed '$d')

if [ "${FIDO2_AUTH_CONFIG_HTTP_CODE}" = "200" ] || [ "${FIDO2_AUTH_CONFIG_HTTP_CODE}" = "204" ]; then
  echo "Authentication configuration (fido2) updated successfully"
else
  echo "Warning: FIDO2 authentication configuration update failed (HTTP ${FIDO2_AUTH_CONFIG_HTTP_CODE})"
  echo "Response: ${FIDO2_AUTH_CONFIG_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${FIDO2_AUTH_CONFIG_RESPONSE_BODY}"
fi
echo ""
...
# Step 8: Update authentication policy (fido2-registration)
echo "Step 8: Updating authentication policy (fido2-registration)..."
AUTH_POLICY_FIDO2_REG_JSON=$(cat "${OUTPUT_DIR}/authentication-policy-fido2-registration.json")

AUTH_POLICY_FIDO2_REG_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-policies/${AUTH_POLICY_FIDO2_REG_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${AUTH_POLICY_FIDO2_REG_JSON}")

AUTH_POLICY_FIDO2_REG_HTTP_CODE=$(echo "${AUTH_POLICY_FIDO2_REG_RESPONSE}" | tail -n1)
AUTH_POLICY_FIDO2_REG_RESPONSE_BODY=$(echo "${AUTH_POLICY_FIDO2_REG_RESPONSE}" | sed '$d')

if [ "${AUTH_POLICY_FIDO2_REG_HTTP_CODE}" = "200" ] || [ "${AUTH_POLICY_FIDO2_REG_HTTP_CODE}" = "204" ]; then
  echo "Authentication policy (fido2-registration) updated successfully"
else
  echo "Warning: FIDO2 registration policy update failed (HTTP ${AUTH_POLICY_FIDO2_REG_HTTP_CODE})"
  echo "Response: ${AUTH_POLICY_FIDO2_REG_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${AUTH_POLICY_FIDO2_REG_RESPONSE_BODY}"
fi
echo ""
