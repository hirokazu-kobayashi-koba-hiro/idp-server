...
[ ! -f "$JSON_FILE" ] && echo "❌ IdentityVerification config file not found: $JSON_FILE" && exit 1
...
echo "🔍 Checking if identity-verification config exists: $CONFIG_ID"
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X GET \
  "${BASE_URL}/v1/management/organizations/${ORGANIZATION_ID}/tenants/${TENANT_ID}/identity-verification-configurations/${CONFIG_ID}" \
...
if [ "$HTTP_CODE" == "200" ]; then
  ...
  METHOD="PUT"
  URL="${BASE_URL}/v1/management/organizations/${ORGANIZATION_ID}/tenants/${TENANT_ID}/identity-verification-configurations/${CONFIG_ID}${DRY_RUN_PARM}"
elif [ "$HTTP_CODE" == "404" ]; then
  echo "🆕 Config not found. Registering new one..."
  METHOD="POST"
  URL="${BASE_URL}/v1/management/organizations/${ORGANIZATION_ID}/tenants/${TENANT_ID}/identity-verification-configurations${DRY_RUN_PARM}"
else
