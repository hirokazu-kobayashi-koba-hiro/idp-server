...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X GET \
  "${BASE_URL}/v1/management/organizations/${ORGANIZATION_ID}/tenants/${TENANT_ID}/authentication-policies/${CONFIG_ID}" \
...
if [ "$HTTP_CODE" == "200" ]; then
  ...
  METHOD="PUT"
  URL="${BASE_URL}/v1/management/organizations/${ORGANIZATION_ID}/tenants/${TENANT_ID}/authentication-policies/${CONFIG_ID}${DRY_RUN_PARM}"
elif [ "$HTTP_CODE" == "404" ]; then
  echo "🆕 Config not found. Registering new one..."
  METHOD="POST"
  URL="${BASE_URL}/v1/management/organizations/${ORGANIZATION_ID}/tenants/${TENANT_ID}/authentication-policies${DRY_RUN_PARM}"
else
...
if [[ "$METHOD" == "POST" && "$FINAL_HTTP_CODE" == "201" ]]; then
  echo "$BODY"
  echo "✅ Authentication policy successfully registered"
  ...
  echo "✅ Authentication policy successfully updated"
else
  echo "$BODY"
  echo "❌ Authentication policy $METHOD failed"
  exit 1
fi
