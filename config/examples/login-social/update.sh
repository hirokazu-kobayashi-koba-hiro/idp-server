...
# Login (Social) Example - Update Script
...
echo "=========================================="
echo "Login (Social) Example Update"
echo "=========================================="
...
CLIENT_JSON=$(jq --arg fed_id "${FEDERATION_ID}" '. + {"available_federations": [$fed_id]}' "${CLIENT_FILE}")
...
# Step 5: Update federation configuration
echo "Step 5: Updating federation configuration..."
FEDERATION_JSON=$(cat "${SCRIPT_DIR}/federation-config-google.json")

FEDERATION_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/federation-configurations/${FEDERATION_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${FEDERATION_JSON}")
...
if [ "${FEDERATION_HTTP_CODE}" = "200" ] || [ "${FEDERATION_HTTP_CODE}" = "204" ]; then
  echo "Federation configuration updated successfully"
else
  echo "Warning: Federation configuration update failed (HTTP ${FEDERATION_HTTP_CODE})"
  echo "Response: ${FEDERATION_RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${FEDERATION_RESPONSE_BODY}"
fi
echo ""
