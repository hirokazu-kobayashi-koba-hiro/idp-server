...
# Login (Social) Example - Delete Script
...
echo "=========================================="
echo "Login (Social) Example Delete"
echo "=========================================="
...
# Step 2: Delete federation configuration
echo "Step 2: Deleting federation configuration..."
FEDERATION_DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE \
  "${AUTHORIZATION_SERVER_URL}/v1/management/tenants/${PUBLIC_TENANT_ID}/federation-configurations/${FEDERATION_ID}" \
  -H "Authorization: Bearer ${SYSTEM_ACCESS_TOKEN}")

FEDERATION_DELETE_HTTP_CODE=$(echo "${FEDERATION_DELETE_RESPONSE}" | tail -n1)

if [ "${FEDERATION_DELETE_HTTP_CODE}" = "204" ]; then
  echo "Federation configuration deleted successfully"
elif [ "${FEDERATION_DELETE_HTTP_CODE}" = "404" ]; then
  echo "Warning: Federation configuration not found (may already be deleted)"
else
  echo "Warning: Federation configuration deletion failed (HTTP ${FEDERATION_DELETE_HTTP_CODE})"
fi
echo ""

# Step 3: Delete application client
echo "Step 3: Deleting application client..."
