...
# Login (Social) Example - Verification Script
#
# Verifies that the example environment works correctly by performing:
#   1. Discovery endpoint check + federation configuration existence
#   2. Authorization request
#   3. User registration via initial-registration
#   4. Consent grant (authorize)
#   5. Token exchange (authorization code → tokens)
#   6. UserInfo endpoint verification
#   7. Token Refresh
#
# Note: Social login (Google) requires browser interaction and cannot be
#       verified automatically. This script verifies the password-based flow
#       and checks that federation configuration exists.
#
# Prerequisites:
...
echo "=========================================="
echo "Login (Social) Example Verification"
echo "=========================================="
echo ""
...
# ============================================================
# Step 1: Discovery Endpoint + Federation Config Check
# ============================================================
...
# Check federation configuration file exists
echo "  Checking federation configuration..."
if [ -f "${SCRIPT_DIR}/federation-config-google.json" ]; then
  FEDERATION_PROVIDER=$(jq -r '.sso_provider' "${SCRIPT_DIR}/federation-config-google.json")
  echo "  Federation config present: ${FEDERATION_PROVIDER}"
  check_result "federation_config" "true"
else
  echo "  Federation config file not found"
  check_result "federation_config" "false"
fi
echo ""
...
echo ""
echo "Note: Social login (Google) requires browser-based testing"
echo "      and cannot be verified automatically."
echo "      To test Google login manually, open:"
echo "      ${TENANT_BASE}/v1/authorizations?response_type=code&client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=openid%20profile%20email&state=test"
echo ""
