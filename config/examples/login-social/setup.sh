...
# Login (Social) Example Setup Script
# Sets up a complete social login (Google + password) environment from scratch.
...
echo "=========================================="
echo "Login (Social) Example Setup"
echo "=========================================="
...
# Step 8: Create federation configuration (Google)
echo "Step 8: Creating federation configuration (Google)..."

FEDERATION_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/federation-configurations" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${SCRIPT_DIR}/federation-config-google.json")

HTTP_CODE=$(echo "${FEDERATION_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${FEDERATION_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Federation configuration created"
else
  echo "Warning: Federation configuration creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""

# Step 9: Update client to add available_federations
echo "Step 9: Updating client with federation settings..."

FEDERATION_ID=$(jq -r '.id' "${SCRIPT_DIR}/federation-config-google.json")
CLIENT_WITH_FEDERATION=$(jq --arg fed_id "${FEDERATION_ID}" '. + {"available_federations": [$fed_id]}' "${CLIENT_FILE}")

CLIENT_UPDATE_RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/clients/${CLIENT_ID}" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d "${CLIENT_WITH_FEDERATION}")

HTTP_CODE=$(echo "${CLIENT_UPDATE_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${CLIENT_UPDATE_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "204" ]; then
  echo "Client updated with federation settings"
else
  echo "Warning: Client update failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""
...
echo "   Federation Config ID: ${FEDERATION_ID}"
echo ""
...
echo "   Client Alias:   login-social-webapp"
...
echo "   Auth policy:      password + social login (Google)"
echo "   Federation:       Google OIDC"
echo ""
echo "Google OAuth Setup:"
echo "   federation-config-google.json の client_id / client_secret を"
echo "   Google Cloud Console で取得した値に置き換えてください。"
echo "   Redirect URI: https://api.local.test/${PUBLIC_TENANT_ID}/v1/authorizations/federations/oidc/callback"
echo ""
echo "OIDC Discovery:"
...
echo ""
echo "   2. Register or login with email/password, or use Google login"
echo ""
