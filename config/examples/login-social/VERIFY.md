...
## Step 1: Discovery Endpoint + Federation Config 確認

OpenID Connect の Discovery エンドポイントとフェデレーション設定を確認します。
...
### Federation 設定ファイルの確認

```bash
jq . federation-config-google.json
```

### 確認ポイント

- `federation-config-google.json` が存在すること
- `sso_provider` フィールドが設定されていること
...
上記の CLI テストではパスワードベースの認証フローのみ検証しています。
Google ソーシャルログインを手動でテストするには、以下の URL をブラウザで開いてください:
...
> Google OAuth の Client ID/Secret が正しく設定されている必要があります。
...
| 1 | issuer が正しい | |
| 1 | federation-config-google.json が存在する | |
