...
if [ "${HTTP_CODE}" = "201" ]; then
  ...
  echo "   API Domain:  https://api.local.test"
  echo "   Auth Domain: https://auth.local.test"
  echo ""
  echo "Admin User:"
  ...
  echo "   Redirect URI:  https://auth.local.test/callback/"
  echo ""
  if [ -n "${WEB_CLIENT_ID}" ]; then
    ...
    echo "   Redirect URI:  https://sample.local.test/api/auth/callback/idp-server"
    echo ""
  fi
  echo "Test Authorization Code Flow:"
  echo "   1. Configure /etc/hosts (or use dnsmasq):"
  echo "      127.0.0.1 api.local.test"
  echo "      127.0.0.1 auth.local.test"
  echo "      127.0.0.1 sample.local.test"
  echo ""
  echo "   2. Open browser:"
  echo "      open \"https://api.local.test/${TENANT_ID}/v1/authorizations?response_type=code&client_id=fd385eef-19e5-5150-93c9-6debf1a901fa&redirect_uri=https://auth.local.test/callback/&scope=openid%20profile%20email&state=test-state\""
  echo ""
  ...
  if [ -n "${WEB_CLIENT_ID}" ]; then
    ...
    echo "   open https://sample.local.test"
    echo ""
  fi
  ...
  echo "   RP ID:   local.test"
  echo "   Origin:  https://auth.local.test"
  echo ""
else
