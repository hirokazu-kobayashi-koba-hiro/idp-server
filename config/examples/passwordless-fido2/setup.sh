...
# Passwordless (FIDO2/WebAuthn) Example Setup Script
# Sets up a complete passwordless (FIDO2/WebAuthn) environment from scratch.
...
echo "=========================================="
echo "Passwordless (FIDO2/WebAuthn) Example Setup"
echo "=========================================="
...
# Step 4: Create public tenant (password policy + session config + device rule)
...
if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Authentication configuration (initial-registration) created"
else
  echo "Warning: Authentication configuration (initial-registration) creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""
...
FIDO2_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/authentication-configurations" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${SCRIPT_DIR}/authentication-config-fido2.json")

HTTP_CODE=$(echo "${FIDO2_CONFIG_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${FIDO2_CONFIG_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Authentication configuration (fido2) created"
else
  echo "Warning: Authentication configuration (fido2) creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""

# Step 7: Create authentication policy (fido2 or password - oauth flow)
...
if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Authentication policy (oauth) created"
else
  echo "Warning: Authentication policy (oauth) creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""

# Step 8: Create authentication policy (fido2-registration flow)
...
FIDO2_REG_POLICY_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  ...
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${SCRIPT_DIR}/authentication-policy-fido2-registration.json")

HTTP_CODE=$(echo "${FIDO2_REG_POLICY_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${FIDO2_REG_POLICY_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Authentication policy (fido2-registration) created"
else
  echo "Warning: Authentication policy (fido2-registration) creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""
...
echo "   Client Alias:   fido2-webapp"
echo "   Client Secret:  ${CLIENT_SECRET}"
echo "   Redirect URI:   ${REDIRECT_URI}"
echo ""
echo "Settings:"
echo "   Auth policy:      FIDO2/WebAuthn OR password (migration pattern)"
echo "   FIDO2 RP ID:      local.test"
echo "   Device rule:      max 5 devices, HS256, 24h secret expiry"
...
echo ""
echo "FIDO2 Registration Policy:"
echo "   Flow:             fido2-registration"
echo "   Required:         FIDO2 device registration success"
echo ""
echo "OIDC Discovery:"
...
echo ""
echo "   2. Register with email/password, then register FIDO2 device"
echo ""
