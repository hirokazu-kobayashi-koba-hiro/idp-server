...
# eKYC (Identity Verification) Example Setup Script
# Sets up a complete eKYC environment with identity verification from scratch.
...
echo "=========================================="
echo "eKYC (Identity Verification) Example Setup"
echo "=========================================="
...
# Step 4: Create public tenant (password policy + session config + verified_claims)
...
# Step 8: Create identity verification configuration
echo "Step 8: Creating identity verification configuration..."

IV_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  "${ORG_BASE_URL}/${PUBLIC_TENANT_ID}/identity-verification-configurations" \
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${SCRIPT_DIR}/identity-verification-config.json")

HTTP_CODE=$(echo "${IV_CONFIG_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${IV_CONFIG_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Identity verification configuration created"
else
  echo "Warning: Identity verification configuration creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""
...
echo "   verified_claims_supported: true"
echo "   Identity verification:     transfers scope triggers eKYC"
echo ""
echo "OIDC Discovery:"
echo "   ${AUTHORIZATION_SERVER_URL}/${PUBLIC_TENANT_ID}/.well-known/openid-configuration"
echo ""
echo "Test Authorization Code Flow (with identity verification):"
echo "   1. Open browser:"
echo "      ${AUTHORIZATION_SERVER_URL}/${PUBLIC_TENANT_ID}/v1/authorizations?response_type=code&client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=openid%20profile%20email%20transfers&state=test-state"
echo ""
echo "   2. Register or login with email/password"
echo ""
echo "   3. Identity verification will be triggered by the 'transfers' scope"
echo ""
