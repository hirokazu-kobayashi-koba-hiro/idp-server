...
# eKYC (Identity Verification) Example - Verification Script
#
# Verifies that the example environment works correctly by performing:
#   1. Discovery endpoint check + verified_claims_supported confirmation
...
echo "=========================================="
echo "eKYC (Identity Verification) Example Verification"
echo "=========================================="
echo ""
...
# ============================================================
# Step 1: Discovery Endpoint + verified_claims_supported
# ============================================================
...
if [ "${DISCOVERY_HTTP}" = "200" ]; then
  ...
  # Check verified_claims_supported
  VERIFIED_CLAIMS=$(echo "${DISCOVERY_BODY}" | jq -r '.verified_claims_supported // "false"')
  echo "  verified_claims_supported: ${VERIFIED_CLAIMS}"
  if [ "${VERIFIED_CLAIMS}" = "true" ]; then
    check_result "verified_claims_supported" "true"
  else
    echo "  Expected verified_claims_supported: true"
    check_result "verified_claims_supported" "false"
  fi
else
