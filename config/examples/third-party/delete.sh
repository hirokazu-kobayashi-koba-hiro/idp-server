...
# Third Party Integration Example - Delete Script
...
echo "=========================================="
echo "Third Party Integration Example Delete"
echo "=========================================="
...
# Step 2: Delete Web application client
echo "Step 2: Deleting Web application client..."
WEB_CLIENT_DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE \
  "${AUTHORIZATION_SERVER_URL}/v1/management/tenants/${PUBLIC_TENANT_ID}/clients/${WEB_CLIENT_ID}" \
...
if [ "${WEB_CLIENT_DELETE_HTTP_CODE}" = "204" ]; then
  echo "Web application client deleted successfully"
elif [ "${WEB_CLIENT_DELETE_HTTP_CODE}" = "404" ]; then
  echo "Warning: Web application client not found (may already be deleted)"
else
  echo "Warning: Web application client deletion failed (HTTP ${WEB_CLIENT_DELETE_HTTP_CODE})"
fi
echo ""

# Step 3: Delete Mobile application client
echo "Step 3: Deleting Mobile application client..."
MOBILE_CLIENT_DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE \
  "${AUTHORIZATION_SERVER_URL}/v1/management/tenants/${PUBLIC_TENANT_ID}/clients/${MOBILE_CLIENT_ID}" \
  -H "Authorization: Bearer ${SYSTEM_ACCESS_TOKEN}")

MOBILE_CLIENT_DELETE_HTTP_CODE=$(echo "${MOBILE_CLIENT_DELETE_RESPONSE}" | tail -n1)

if [ "${MOBILE_CLIENT_DELETE_HTTP_CODE}" = "204" ]; then
  echo "Mobile application client deleted successfully"
elif [ "${MOBILE_CLIENT_DELETE_HTTP_CODE}" = "404" ]; then
  echo "Warning: Mobile application client not found (may already be deleted)"
else
  echo "Warning: Mobile application client deletion failed (HTTP ${MOBILE_CLIENT_DELETE_HTTP_CODE})"
fi
echo ""

# Step 4: Delete M2M client
echo "Step 4: Deleting M2M client..."
M2M_CLIENT_DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE \
  "${AUTHORIZATION_SERVER_URL}/v1/management/tenants/${PUBLIC_TENANT_ID}/clients/${M2M_CLIENT_ID}" \
  -H "Authorization: Bearer ${SYSTEM_ACCESS_TOKEN}")

M2M_CLIENT_DELETE_HTTP_CODE=$(echo "${M2M_CLIENT_DELETE_RESPONSE}" | tail -n1)

if [ "${M2M_CLIENT_DELETE_HTTP_CODE}" = "204" ]; then
  echo "M2M client deleted successfully"
elif [ "${M2M_CLIENT_DELETE_HTTP_CODE}" = "404" ]; then
  echo "Warning: M2M client not found (may already be deleted)"
else
  echo "Warning: M2M client deletion failed (HTTP ${M2M_CLIENT_DELETE_HTTP_CODE})"
fi
echo ""

# Step 5: Delete public tenant
echo "Step 5: Deleting public tenant..."
