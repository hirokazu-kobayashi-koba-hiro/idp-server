...
# MFA (Password + Email OTP) Example Setup Script
# Sets up a complete MFA (password + email OTP) environment from scratch.
...
echo "=========================================="
echo "MFA (Password + Email OTP) Example Setup"
echo "=========================================="
...
EMAIL_AUTH_CONFIG_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  ...
  -H "Authorization: Bearer ${ORG_ACCESS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d @"${SCRIPT_DIR}/authentication-config-email.json")

HTTP_CODE=$(echo "${EMAIL_AUTH_CONFIG_RESPONSE}" | tail -n1)
RESPONSE_BODY=$(echo "${EMAIL_AUTH_CONFIG_RESPONSE}" | sed '$d')

if [ "${HTTP_CODE}" = "200" ] || [ "${HTTP_CODE}" = "201" ]; then
  echo "Authentication configuration (email) created"
else
  echo "Warning: Authentication configuration (email) creation failed (HTTP ${HTTP_CODE})"
  echo "${RESPONSE_BODY}" | jq '.' 2>/dev/null || echo "${RESPONSE_BODY}"
fi
echo ""

# Step 7: Create authentication policy (password AND email MFA)
echo "Step 7: Creating authentication policy (password AND email MFA)..."
...
echo "   Client Alias:   mfa-email-webapp"
...
echo ""
echo "   2. Register or login with email/password + email OTP"
echo ""
