...
#
# This script sets up:
#   1. dnsmasq for *.local.test DNS resolution
#   2. mkcert for local SSL certificates
#
# After running this script:
#   - https://auth.local.test → IDP Server
#   - https://app.local.test  → App View
#
# Usage:
...
echo ""
echo "Step 2: Configuring dnsmasq for *.local.test..."
DNSMASQ_CONF="/opt/homebrew/etc/dnsmasq.conf"
if [[ ! -f "$DNSMASQ_CONF" ]]; then
    DNSMASQ_CONF="/usr/local/etc/dnsmasq.conf"
fi

if ! grep -q "address=/local.test/127.0.0.1" "$DNSMASQ_CONF" 2>/dev/null; then
    echo "address=/local.test/127.0.0.1" >> "$DNSMASQ_CONF"
    echo "  Added local.test → 127.0.0.1"
else
    echo "  local.test is already configured"
fi

echo ""
echo "Step 3: Setting up macOS resolver..."
sudo mkdir -p /etc/resolver
echo "nameserver 127.0.0.1" | sudo tee /etc/resolver/local.test > /dev/null
echo "  Created /etc/resolver/local.test"
...
echo ""
echo "Step 7: Generating SSL certificates for *.local.test..."
mkdir -p "$CERTS_DIR"
cd "$CERTS_DIR"
...
sleep 2
if ping -c 1 auth.local.test &> /dev/null; then
    echo "  ✓ auth.local.test resolves to 127.0.0.1"
else
    echo "  ✗ DNS resolution failed. Try restarting dnsmasq:"
    echo "    sudo brew services restart dnsmasq"
fi

if ping -c 1 app.local.test &> /dev/null; then
    echo "  ✓ app.local.test resolves to 127.0.0.1"
else
    echo "  ✗ DNS resolution failed for app.local.test"
fi
...
echo ""
echo "Access:"
echo "  IDP Server: https://auth.local.test"
echo "  App View:   https://app.local.test"
echo ""
