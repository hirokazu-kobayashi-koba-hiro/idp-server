...
public class MysqlExecutor implements ClientInstanceSqlExecutor {
  ...
  private void appendConditions(
    ...
    if (queries.hasCertificateSerial()) {
      sql.append(" AND JSON_CONTAINS(attestation_evidence, ?, '$.chain.certificates')");
      params.add(jsonConverter.write(Map.of("serial", queries.certificateSerial())));
    }
