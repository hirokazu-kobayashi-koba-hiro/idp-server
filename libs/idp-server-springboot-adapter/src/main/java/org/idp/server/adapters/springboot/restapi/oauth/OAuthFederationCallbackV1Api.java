...
public class OAuthFederationCallbackV1Api implements ParameterTransformable {
  ...
  public ResponseEntity<?> callbackFederation(@PathVariable("federation-type") FederationType federationType, @PathVariable("sso-provider-name") SsoProvider ssoProvider, @RequestBody(required = false) MultiValueMap<String, String> body, HttpServletRequest httpServletRequest) {
    ...
    FederationInteractionResult result = oAuthFlowApi.callbackFederation(federationCallbackRequest.tenantIdentifier(), federationType, ssoProvider, federationCallbackRequest, requestAttributes);
