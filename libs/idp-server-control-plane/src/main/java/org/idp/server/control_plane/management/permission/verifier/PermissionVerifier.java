...
public class PermissionVerifier {
  ...
  public PermissionVerifier(PermissionQueryRepository permissionQueryRepository) {
    this.permissionQueryRepository = permissionQueryRepository;
  }

  public VerificationResult verify(Tenant tenant, Permission permission) {
    Permission permissionByName = permissionQueryRepository.findByName(tenant, permission.name());

    if (permissionByName.exists()) {
      List<String> errors = new ArrayList<>();
      errors.add("Permission '" + permission.name() + "' already exists");
