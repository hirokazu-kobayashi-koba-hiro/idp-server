...
public class SecurityEventHookResultQueryValidator {

  SecurityEventHookResultQueries queries;
  JsonSchemaValidator schemaValidator;

  public SecurityEventHookResultQueryValidator(SecurityEventHookResultQueries queries) {
    this.queries = queries;
    this.schemaValidator =
        new JsonSchemaValidator(ControlPlaneV1SchemaReader.securityEventHookResultQuerySchema());
  }
  ...
  void throwExceptionIfInvalid(JsonSchemaValidationResult result) {
    if (!result.isValid()) {
      throw new InvalidRequestException(
          "Security event hook result query validation failed", result.errors());
    }
  }
}
