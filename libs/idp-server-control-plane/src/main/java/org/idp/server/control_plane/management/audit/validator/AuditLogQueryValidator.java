...
package org.idp.server.control_plane.management.audit.validator;
...
public class AuditLogQueryValidator {
  ...
  public AuditLogQueryValidator(AuditLogQueries queries) {
    this.queries = queries;
    this.schemaValidator =
        new JsonSchemaValidator(ControlPlaneV1SchemaReader.auditLogQuerySchema());
  }
  ...
  void throwExceptionIfInvalid(JsonSchemaValidationResult result) {
    if (!result.isValid()) {
      throw new InvalidRequestException("Audit log query validation failed", result.errors());
    }
  }
}
