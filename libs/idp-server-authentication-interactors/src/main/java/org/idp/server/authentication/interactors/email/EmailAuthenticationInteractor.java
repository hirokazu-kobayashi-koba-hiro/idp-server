...
public class EmailAuthenticationInteractor implements AuthenticationInteractor {
  ...
  public AuthenticationInteractionRequestResult interact(
    ...
    if (schemaDefinition.exists()) {
      ...
      if (!validationResult.isValid()) {
        log.warn(
            "Email authentication request validation failed: error_count={}, errors={}",
    ...
    if (!executionResult.isSuccess()) {
      log.warn(
          "Email authentication failed. status={}, contents={}",
    ...
    Pairs<User, AuthenticationInteractionRequestResult> resolved =
        resolveUserOrError(tenant, transaction, type, email, providerId, userQueryRepository);
   ...
   *
   * <p>{@link EmailAuthenticationChallengeInteractor} already maps this exception to a client error
   ...
   *     when the email maps to multiple users
   */
