...
import org.idp.server.core.openid.identity.email.EmailVerificationCodeSender;
 ...
 *
 * <p>Lives in this module because {@link EmailAuthenticationConfiguration} does; the domain side
 * depends only on the {@link EmailVerificationCodeSender} port.
 */
public class EmailAuthenticationConfigCodeSender implements EmailVerificationCodeSender {
  ...
  @Override
  public int expireSeconds(Tenant tenant) {
    return configuration(tenant).expireSeconds();
  }

  @Override
  public int retryCountLimitation(Tenant tenant) {
  ...
  public boolean send(
    ...
    EmailSendingRequest sendingRequest =
        new EmailSendingRequest(configuration.sender(), targetEmail, template.subject(), body);
