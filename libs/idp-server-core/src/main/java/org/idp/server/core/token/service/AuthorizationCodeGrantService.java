 ...
 *
 * <p>
 * client_id REQUIRED, if the client is not authenticating with the authorization server as
 ...
 *
 * <p>
 * POST /token HTTP/1.1 Host: server.example.com Authorization: Basic czZCaGRSa3F0MzpnWDFmQmF0M2JW
 ...
 *
 * <p>
 * o ensure that the authorization code was issued to the authenticated confidential client, or if
 ...
 *
 * <p>
 * o ensure that the "redirect_uri" parameter is present if the "redirect_uri" parameter was
 ...
 *      Request</a>
 */
public class AuthorizationCodeGrantService implements OAuthTokenCreationService, AccessTokenCreatable, RefreshTokenCreatable, IdTokenCreatable, CNonceCreatable {
  ...
  public AuthorizationCodeGrantService(AuthorizationRequestRepository authorizationRequestRepository, OAuthTokenRepository oAuthTokenRepository, AuthorizationCodeGrantRepository authorizationCodeGrantRepository, AuthorizationGrantedRepository authorizationGrantedRepository) {
  ...
  public OAuthToken create(TokenRequestContext tokenRequestContext, ClientCredentials clientCredentials) {
    TokenRequestCodeGrantValidator validator = new TokenRequestCodeGrantValidator(tokenRequestContext.parameters());
    validator.validate();

    Tenant tenant = tokenRequestContext.tenant();
    AuthorizationCode code = tokenRequestContext.code();
    AuthorizationCodeGrant authorizationCodeGrant = authorizationCodeGrantRepository.find(tenant, code);
    AuthorizationRequest authorizationRequest = authorizationRequestRepository.find(tenant, authorizationCodeGrant.authorizationRequestIdentifier());

    AuthorizationCodeGrantVerifier verifier = new AuthorizationCodeGrantVerifier(tokenRequestContext, authorizationRequest, authorizationCodeGrant, clientCredentials);
    verifier.verify();
    ...
    if (authorizationRequest.isOidcProfile()) {
      IdTokenCustomClaims idTokenCustomClaims = new IdTokenCustomClaimsBuilder().add(authorizationCodeGrant.authorizationCode()).add(accessToken.accessTokenEntity()).add(authorizationRequest.nonce()).add(authorizationRequest.state()).build();
      IdToken idToken = createIdToken(authorizationGrant.user(), authorizationCodeGrant.authentication(), authorizationGrant, idTokenCustomClaims, authorizationRequest.requestedClaimsPayload(), serverConfiguration, clientConfiguration);
  ...
  private void registerOrUpdate(Tenant tenant, AuthorizationGrant authorizationGrant) {
    AuthorizationGranted latest = authorizationGrantedRepository.find(tenant, authorizationGrant.requestedClientId(), authorizationGrant.user());
    ...
    AuthorizationGranted authorizationGranted = new AuthorizationGranted(authorizationGrantedIdentifier, authorizationGrant);
