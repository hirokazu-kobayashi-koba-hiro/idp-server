...
public class RequestedClaimsPayload implements JsonReadable {
  ...
  public List<String> idTokenClaims() {
    ...
    if (idToken.hasName())
      claims.add("name");
    if (idToken.hasGivenName())
      claims.add("given_name");
    if (idToken.hasMiddleName())
      claims.add("middle_name");
    if (idToken.hasNickname())
      claims.add("nickname");
    if (idToken.hasPreferredUsername())
      claims.add("preferred_username");
    if (idToken.hasProfile())
      claims.add("profile");
    if (idToken.hasPicture())
      claims.add("picture");
    if (idToken.hasWebsite())
      claims.add("website");
    if (idToken.hasEmail())
      claims.add("email");
    if (idToken.hasEmailVerified())
      claims.add("email_verified");
    if (idToken.hasGender())
      claims.add("gender");
    if (idToken.hasBirthdate())
      claims.add("birthdate");
    if (idToken.hasZoneinfo())
      claims.add("zoneinfo");
    if (idToken.hasLocale())
      claims.add("locale");
    if (idToken.hasPhoneNumber())
      claims.add("phone_number");
    if (idToken.hasPhoneNumberVerified())
      claims.add("phone_number_verified");
    if (idToken.hasAddress())
      claims.add("address");
    if (idToken.hasUpdatedAt())
      claims.add("updated_at");
