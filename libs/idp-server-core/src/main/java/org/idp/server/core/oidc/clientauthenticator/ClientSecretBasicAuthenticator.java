 ...
 *
 * <p>
 * Clients in possession of a client password MAY use the HTTP Basic authentication scheme as
...
class ClientSecretBasicAuthenticator implements ClientAuthenticator {
  ...
  public ClientCredentials authenticate(BackchannelRequestContext context) {
    ...
    return new ClientCredentials(requestedClientId, ClientAuthenticationType.client_secret_basic, clientSecret, new ClientAuthenticationPublicKey(), new ClientAssertionJwt(), new ClientCertification());
  }
  ...
  void throwExceptionIfUnMatchClientSecret(BackchannelRequestContext context) {
    ...
    if (!clientConfiguration.matchClientSecret(clientSecretBasic.clientSecret().value())) {
      throw new ClientUnAuthorizedException("client authentication type is client_secret_basic, but request client_secret does not match client_secret");
    }
  }

  void throwExceptionIfNotContainsClientSecretBasic(BackchannelRequestContext context) {
    if (!context.hasClientSecretBasic()) {
      throw new ClientUnAuthorizedException("client authentication type is client_secret_basic, but request does not contains client_secret_basic");
    }
  }
}
