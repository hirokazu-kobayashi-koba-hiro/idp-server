...
public class ResourceOwnerPasswordGrantValidator {
  ...
  void throwExceptionIfUnSupportedGrantTypeWithServer() {
    if (!tokenRequestContext.isSupportedGrantTypeWithServer(GrantType.password)) {
      throw new TokenBadRequestException("unsupported_grant_type", "this request grant_type is password, but authorization server does not support");
    }
  }
  ...
  void throwExceptionIfUnSupportedGrantTypeWithClient() {
    if (!tokenRequestContext.isSupportedGrantTypeWithClient(GrantType.password)) {
      throw new TokenBadRequestException("unauthorized_client", "this request grant_type is password, but client does not support");
    }
  }

  void throwExceptionIfNotContainsUsername() {
    if (!tokenRequestContext.hasUsername()) {
      throw new TokenBadRequestException("token request does not contains username, password grant must contains username");
    }
  }

  void throwExceptionIfNotContainsPassword() {
    if (!tokenRequestContext.hasPassword()) {
      throw new TokenBadRequestException("token request does not contains password, password grant must contains password");
    }
  }
