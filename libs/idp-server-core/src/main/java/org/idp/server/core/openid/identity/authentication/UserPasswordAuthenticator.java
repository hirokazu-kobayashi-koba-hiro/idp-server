...
import org.idp.server.core.openid.oauth.type.oauth.Password;
import org.idp.server.core.openid.oauth.type.oauth.Username;
...
public class UserPasswordAuthenticator implements PasswordCredentialsGrantDelegate {
  ...
  public User findAndAuthenticate(Tenant tenant, Username username, Password password) {
    User user = userQueryRepository.findByPreferredUsername(tenant, "idp-server", username.value());
    if (!user.exists()) {
