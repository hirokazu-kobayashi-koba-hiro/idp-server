 ...
 *
 * <p>
 * The CibaRequestHandler class processes backchannel authentication requests as per the CIBA
 ...
 *
 * <p>
 * Typical usage includes receiving a CibaRequest, processing it to determine the authentication
...
public class CibaRequestHandler {
   ...
   */
  public CibaRequestHandler(BackchannelAuthenticationRequestRepository backchannelAuthenticationRequestRepository, CibaGrantRepository cibaGrantRepository, ServerConfigurationRepository serverConfigurationRepository, ClientConfigurationRepository clientConfigurationRepository) {
  ...
  public CibaRequestContext handleRequest(CibaRequest request) {
    ...
    CibaRequestContext context = cibaRequestContextCreator.create(request.clientSecretBasic(), request.toClientCert(), parameters, serverConfiguration, clientConfiguration);
  ...
  public CibaIssueResponse handleIssueResponse(CibaIssueRequest request) {
    ...
    BackchannelAuthenticationResponse response = new BackchannelAuthenticationResponseBuilder().add(new AuthReqId(UUID.randomUUID().toString())).add(context.expiresIn()).add(context.interval()).build();

    backchannelAuthenticationRequestRepository.register(tenant, context.backchannelAuthenticationRequest());

    User user = request.user();
    CibaGrantFactory cibaGrantFactory = new CibaGrantFactory(context, response, user, new Authentication());
  ...
  public BackchannelAuthenticationRequest handleGettingRequest(Tenant tenant, BackchannelAuthenticationRequestIdentifier backchannelAuthenticationRequestIdentifier) {
    return backchannelAuthenticationRequestRepository.find(tenant, backchannelAuthenticationRequestIdentifier);
  }
}
