...
public enum IdentityVerificationApplicationStatus {
  REQUESTED("requested"), APPLYING("applying"), EXAMINATION_PROCESSING("examination_processing"), APPROVED("approved"), REJECTED("rejected"), EXPIRED("expired"), CANCELLED("cancelled"), UNDEFINED(""), UNKNOWN("unknown");
  ...
  public static IdentityVerificationApplicationStatus of(String value) {
    for (IdentityVerificationApplicationStatus status : IdentityVerificationApplicationStatus.values()) {
  ...
  public static boolean isRejected(IdentityVerificationRequest request, IdentityVerificationProcessConfiguration processConfig) {
    ...
    if (Objects.equals(type, "boolean")) {
      String field = (String) rejectedConditionSchema.getOrDefault("field", "");
      Boolean expectedValue = (Boolean) rejectedConditionSchema.getOrDefault("expected_value", false);
