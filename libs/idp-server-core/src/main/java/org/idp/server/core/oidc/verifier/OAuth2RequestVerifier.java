...
public class OAuth2RequestVerifier implements AuthorizationRequestVerifier {
   ...
   * endpoint URI MAY include an "application/x-www-form-urlencoded" formatted (per Appendix B) query
   * component ([RFC3986] Section 3.4), which MUST be retained when adding additional query
  ...
  void throwExceptionIfRedirectUriContainsFragment(OAuthRequestContext context) {
    try {
      ...
      if (uri.hasFragment()) {
        throw new OAuthBadRequestException("invalid_request", String.format("redirect_uri must not fragment (%s)", context.redirectUri().value()));
      }
    } catch (InvalidUriException exception) {
      throw new OAuthBadRequestException("invalid_request", String.format("authorization request redirect_uri is invalid (%s)", context.redirectUri().value()));
    }
  }
   ...
   *
   * <p>
   * If multiple redirection URIs have been registered, if only part of the redirection URI has been
   * registered, or if no redirection URI has been registered, the client MUST include a redirection
   * URI with the authorization request using the "redirect_uri" request parameter.
   *
   * <p>
   * When a redirection URI is included in an authorization request, the authorization server MUST
   * compare and match the value received against at least one of the registered redirection URIs (or
   * URI components) as defined in [RFC3986] Section 6, if any redirection URIs were registered. If
   * the client registration included the full redirection URI, the authorization server MUST compare
  ...
  void throwExceptionIfUnMatchRedirectUri(OAuthRequestContext context) {
    ...
    if (!registeredRedirectUris.containsWithNormalizationAndComparison(context.redirectUri().value())) {
      throw new OAuthBadRequestException("invalid_request", String.format("authorization request redirect_uri does not match registered redirect uris (%s)", context.redirectUri().value()));
    }
  }
  ...
  void throwExceptionIfMultiRegisteredRedirectUri(OAuthRequestContext context) {
    if (context.isMultiRegisteredRedirectUri()) {
      throw new OAuthBadRequestException("invalid_request", "on multiple registered redirect uris, authorization request redirect_uri must contains");
    }
  }
}
