...
public class OAuthRequestValidator {
  ...
  void throwExceptionIfNotContainsClientId() {
    if (!oAuthRequestParameters.hasClientId()) {
      throw new OAuthBadRequestException("invalid_request", "authorization request must contains client_id");
    }
  }
   ...
   *
   * @see <a href="https://www.rfc-editor.org/rfc/rfc6749#section-3.1">3.1. Authorization Endpoint</a>
   */
  void throwExceptionIfDuplicateValue() {
    ...
    if (!filteredKeys.isEmpty()) {
      ...
      throw new OAuthBadRequestException("invalid_request", String.format("authorization request must not contains duplicate value; keys (%s)", keysValue));
    }
  }
}
