...
public class ClientNotificationService implements AccessTokenCreatable, IdTokenCreatable, RefreshTokenCreatable {
  ...
  public ClientNotificationService(BackchannelAuthenticationRequestRepository backchannelAuthenticationRequestRepository, AuthorizationGrantedRepository authorizationGrantedRepository, OAuthTokenRepository oAuthTokenRepository, ClientNotificationGateway clientNotificationGateway) {
  ...
  public void notify(Tenant tenant, CibaGrant cibaGrant, ServerConfiguration serverConfiguration, ClientConfiguration clientConfiguration) {
    ...
    ClientNotificationRequestBodyBuilder builder = new ClientNotificationRequestBodyBuilder().add(cibaGrant.authReqId());

    if (backchannelAuthenticationRequest.isPingMode()) {
      ClientNotificationRequest clientNotificationRequest = new ClientNotificationRequest(clientConfiguration.backchannelClientNotificationEndpoint(), builder.build(), backchannelAuthenticationRequest.clientNotificationToken().value());
      clientNotificationGateway.notify(clientNotificationRequest);
    }

    if (backchannelAuthenticationRequest.isPushMode()) {
      AccessToken accessToken = createAccessToken(cibaGrant.authorizationGrant(), serverConfiguration, clientConfiguration, new ClientCredentials());
      ...
      IdToken idToken = createIdToken(cibaGrant.user(), new Authentication(), cibaGrant.authorizationGrant(), idTokenCustomClaims, new RequestedClaimsPayload(), serverConfiguration, clientConfiguration);
      builder.add(accessToken.accessTokenEntity()).add(TokenType.Bearer).add(new ExpiresIn(serverConfiguration.accessTokenDuration())).add(refreshToken.refreshTokenEntity()).add(idToken);
      ...
      OAuthToken oAuthToken = new OAuthTokenBuilder(identifier).add(accessToken).add(refreshToken).add(idToken).build();
