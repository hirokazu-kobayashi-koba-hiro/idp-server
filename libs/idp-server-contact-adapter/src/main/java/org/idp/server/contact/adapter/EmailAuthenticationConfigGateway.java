...
/**
 * Drives the email code exchange using the tenant's existing {@code type: "email"} authentication
 ...
 * </ul>
 */
public class EmailAuthenticationConfigGateway
  ...
  LoggerWrapper log = LoggerWrapper.getLogger(EmailAuthenticationConfigGateway.class);

  public EmailAuthenticationConfigGateway(
      AuthenticationConfigurationQueryRepository configurationQueryRepository,
      EmailSenders emailSenders,
      ExternalContactVerificationExchange externalExchange) {
  ...
  public ContactChallengeStart start(
    ...
    EmailAuthenticationConfiguration configuration = localConfiguration(execution);
  ...
  private AuthenticationExecutionConfig challengeExecution(Tenant tenant) {
    return interactionConfig(tenant, "email-authentication-challenge").execution();
  }

  private AuthenticationExecutionConfig verifyExecution(Tenant tenant) {
    return interactionConfig(tenant, "email-authentication").execution();
  }
  ...
  private EmailAuthenticationConfiguration localConfiguration(
    ...
    return jsonConverter.read(execution.details(), EmailAuthenticationConfiguration.class);
  }
}
