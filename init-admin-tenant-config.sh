...
# Authorization UI (app-view). The signin page calls the authentication API from this origin,
# so it has to be in allow_origins as well.
#
# The template pairs this with cookie_same_site=Lax, which assumes the UI and the API stay on the
# same site (auth.local.test and api.local.test share the registrable domain local.test). Putting
# them on different registrable domains makes the browser withhold the AUTH_SESSION cookie and the
# authorization flow fails with auth_session_mismatch. Such a deployment needs
# cookie_same_site=None *together with* use_secure_cookie=true - None alone is rejected outright.
export UI_BASE_URL="${UI_BASE_URL:-https://auth.local.test}"
export ALLOW_ORIGINS='"'"${UI_BASE_URL}"'","http://localhost:3000","http://localhost:3100","http://localhost:5173","https://idp-auth-frontend-ecru.vercel.app","https://idp-auth-frontend-diqt.vercel.app"'
