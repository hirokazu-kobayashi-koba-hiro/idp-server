...
| 2 | User Code 要否 | `true` / `false`（デフォルト） | 認可サーバー `backchannel_user_code_parameter_supported`, `required_backchannel_auth_user_code` |
...
CIBA の `user_code` パラメータは、CIBAリクエストを送信したクライアントが本当にユーザーの意図を持っているかを追加検証する仕組み。

**現在のidp-serverの実装**: `backchannel_auth_user_code_type = "password"` のみサポート。
`UserCodeAsPasswordVerifier` が `user_code` の値をユーザーのパスワードハッシュと照合する。

つまり **user_code にはユーザーのパスワードを渡す**。
...
| 設定 | 場所 | 説明 |
|------|------|------|
| `backchannel_user_code_parameter_supported` | 認可サーバー | Discovery に表示（user_code サポート宣言） |
| `required_backchannel_auth_user_code` | 認可サーバー extension | `true` の場合、user_code なしのCIBAリクエストを拒否 |
| `backchannel_user_code_parameter` | クライアント設定 | クライアント単位でuser_codeを有効化 |

**E2Eテストでの使い方**: `userCode: userPassword` を渡す（パスワードがuser_code）。
