...
| Null Byte Injection | CWE-626 | High | NullByteRejectFilter（Servlet Filter） |
...
`NullByteRejectFilter.java` — Servlet Filterで全APIを一括防御:
...
- 検出時: 400 Bad Request + ERRORレベルセキュリティログ出力
...
public class NullByteRejectFilter extends OncePerRequestFilter {
  ...
  // 検出時: {"error":"invalid_request","error_description":"Invalid parameter value"}
}
```
...
```
e2e/src/tests/spec/null_byte_injection.test.js
```
...
# Null Byte Injection
npm test -- --testPathPattern="null_byte_injection"
```
