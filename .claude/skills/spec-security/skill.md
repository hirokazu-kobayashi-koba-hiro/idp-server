...
1st factor の DB検索優先化に加え、**password を 2nd factor（`requires_user: true`）に使う場合**は、検証対象を入力 username ではなく**セッションの認証済みユーザーに固定**する。これがないと、1段目を被害者として通過後、2段目に**別アカウントの正しい credential** を提示して被害者としてログインできてしまう（バインド欠落 = identifier switching の別経路）。

`PasswordAuthenticationInteractor`（2nd factor）:

- `requires_user && hasUser`: execution request の `username` / `provider_id` を `transaction.user()` の値に上書きしてから executor を呼ぶ → パスワードは**本人の hash に対して**照合される
...
- **Query parameters / form-urlencoded body**: `getParameterNames()` / `getParameterValues()` でチェック
