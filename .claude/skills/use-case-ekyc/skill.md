...
| ATにverified_claims含有（選択モード） | `access_token_selective_verified_claims` | `true` |
...
設定完了後の動作確認・設定カスタマイズには以下を参照:

- `config/templates/use-cases/ekyc/VERIFY.md` - 基本動作確認（認証フロー → eKYC申請 → verified_claims）
- `config/templates/use-cases/ekyc/VERIFY-CONFIG-CHANGES.md` - 設定変更の実験（mock↔http_request、リトライ、condition等）
- `config/templates/use-cases/ekyc/VERIFY-CONFIG-CHANGES-ADVANCED.md` - マッピング関数リファレンス（全20関数）
