...
| スキル | 用途 |
|--------|------|
| `/use-case-setup` | **ユースケース別セットアップ**（ログイン、MFA、パスワードレス、eKYC等をテンプレートで構築） |

`/use-case-setup` を実行すると、ヒアリング → 環境変数マッピング → setup.sh 実行の流れで idp-server の設定を自動構築できます。ローカル環境で動かしてみたい場合はここから始めてください。
...
| `/spec-authentication` | 認証機能実装 |
| `/spec-authorization` | 認可エンドポイント |
| `/spec-token` | トークン発行・管理 |
| `/dev-control-plane` | 管理API開発 |
| `/spec-federation` | 外部IdP連携 |
| `/spec-ciba` | CIBA実装 |
| `/spec-identity-verification` | 身元確認機能 |
...
1. **Phase 1-3** を完了したら、`/use-case-setup` でローカル環境にユースケースを構築してみる
