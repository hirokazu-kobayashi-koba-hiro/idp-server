...
| # | ユースケース | スキル名 | テンプレート |
|---|------------|---------|-------------|
| 1 | ログイン（パスワードのみ） | `use-case-login` | `config/templates/use-cases/login-password-only/` |
| 2 | MFA（多要素認証） | `use-case-mfa` | （未実装） |
| 3 | パスワードレス認証 | `use-case-passwordless` | （未実装） |
| 4 | 身元確認/eKYC | `use-case-ekyc` | （未実装） |
| 5 | サードパーティ連携 | `use-case-third-party` | （未実装） |
