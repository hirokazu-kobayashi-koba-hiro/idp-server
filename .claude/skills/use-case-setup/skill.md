...
| `tenant.cors_config`（`allow_headers`, `allow_methods`, `allow_credentials` 含む） | UI→APIのCORS許可 |
...
| 5 | `signin_page` が正しいパス（`/signin/`） | テナント `ui_config` | パスの末尾スラッシュ忘れ |
...
| モバイルデバイスで生体認証（FIDO-UAF）したい | MFA FIDO-UAF（パスワード + デバイス生体認証） |
