...
`standard` / `facebook` はボディ以外に `$.http_request.status_code` / `$.http_request.response_headers.*` も参照できる。`status_code` は以前 **`staus_code`** というタイポで出力されていた（#1800 で修正）。互換のため両方出力しているが、**v1.0.0 で `staus_code` は削除**。
