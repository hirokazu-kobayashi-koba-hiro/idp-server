...
|---------|-------|------|----------------|
| `api.local.test` | 443 | 通常HTTPS | なし |
| `mtls.api.local.test` | 443 | mTLS | optional |
...
**期待される出力**:
```
Testing mTLS connection to https://mtls.api.local.test/67e7eae6-62b0-4500-9eff-87459f63fc66/health
...
curl -v \
  ...
  --key test-client.key \
  https://mtls.api.local.test/67e7eae6-62b0-4500-9eff-87459f63fc66/v1/health

# OpenID Configuration
curl \
  --cert test-client.pem \
  --key test-client.key \
  https://mtls.api.local.test/67e7eae6-62b0-4500-9eff-87459f63fc66/.well-known/openid-configuration
```
...
curl https://mtls.api.local.test/67e7eae6-62b0-4500-9eff-87459f63fc66/v1/tokens -d "..."
```
...
```bash
curl --cert client.pem --key client.key https://mtls.api.local.test/...
```
...
```bash
curl -k --cert client.pem --key client.key https://mtls.api.local.test/...
```
