...
:::warning 本ドキュメントは未完成
v0.12.0（2026-07-06 リリース）以降、main には **29 件**のマージがある。本ドキュメントに登録済みなのは下表の分のみで、**残りの変更の影響有無は未確認**。リリース前に `git log v0.12.0..main --merges` を洗い出すこと。
:::
...
この一覧は変更前後で共通で、本変更は「列挙されたコードが透過するようになった」もの。
