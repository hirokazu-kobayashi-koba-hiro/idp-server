...
上記の「認証委譲方式」は旧システムを最終的に停止するための段階的移行パターンです。一方、**旧IDサービスを長期的に残しつつ、idp-serverで認証機能を拡張する**パターンもあります。

### ユースケース

- 旧IDサービスのバックエンド認可（APIゲートウェイ等）はそのまま活かしたい
- idp-serverを追加して FIDO認証や eKYC を提供したい
- モバイルアプリは idp-server で認証し、バックエンドは旧アクセストークンで既存の認可基盤を継続利用
...
| 項目 | 認証委譲方式（Step 1〜3） | フェデレーション共存方式 |
|------|------------------------|---------------------|
| 旧システムの扱い | 最終的に停止する | **長期的に共存する** |
| 認証の流れ | idp-server → 旧サービスへHTTP委譲 | 旧サービスをOIDCプロバイダーとしてフェデレーション |
| 旧アクセストークン | 取得しない | **Introspectionで参照可能** |
| バックエンドの認可 | idp-server のトークンに切り替え | 旧トークンで既存の認可基盤を継続利用 |
| 主な用途 | 旧システムの廃止 | 旧システムの活用 + 認証機能の拡張 |

### ポイント

- 旧IDサービスが **OIDCプロバイダー**（またはOAuth 2.0認可サーバー）として動作する必要がある
- 旧IDサービスがOIDC/OAuthに対応していない場合は、「認証委譲方式（Step 1〜3）」を使用
- アクセストークンは **opaque（識別子型）** を使用する（JWT では旧トークンがペイロードに平文で含まれるためセキュリティ上非対応）

> 📖 設定手順の詳細は [How-to: SSOクレデンシャルのアクセストークン埋め込み](../content_05_how-to/phase-4-extensions/06-sso-credentials-access-token.md) を参照してください。
