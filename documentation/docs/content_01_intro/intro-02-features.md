...
| **セルフサービス**    | パスワード変更・リセット                 | ✅    | 認証済みユーザーが自分のパスワードを変更。忘れた場合はメール認証によるリセットに対応。                    |
|                 | 連絡先（Email / 電話番号）の確認・変更    | ✅    | 到達性を確認コードで証明してから確定。変更は専用スコープで保護し、旧値への通知・変更ポリシー（認証条件 / 直近認証）に対応。 |
|                 | MFAデバイスの登録                   | ✅    | FIDO2 / Passkey / FIDO-UAF などの認証デバイスを自分で登録。                       |
|                 | 認証デバイスの更新・削除                 | ✅    | 登録済み認証デバイスのメタデータ更新と削除。                                          |
|                 | アカウント削除                      | ✅    | 認証済みユーザー自身によるアカウント削除。                                           |
|                 | プロフィール取得・更新                  | ⚠️   | 識別子以外の属性（氏名・生年月日・ロケール等）の取得と更新。                                  |
