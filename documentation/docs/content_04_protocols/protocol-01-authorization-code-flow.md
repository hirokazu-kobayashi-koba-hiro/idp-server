...
ブラウザとは別の認証デバイス（スマートフォンアプリ等）に認証を委ねる場合、サインイン画面とデバイスが別チャネルで進行するデカップルド型になります。開始時にクライアント認証が無いため、プッシュ疲労対策としてナンバーマッチングを併用します。

詳細は [認可コードフロー デバイス認証拡張](./protocol-07-authorization-code-device-authentication.md) を参照してください。
...
### 関連ドキュメント
- [マルチテナント](../content_03_concepts/01-foundation/concept-03-multi-tenant.md) - テナント設定の詳細
- [身元確認済みID](../content_03_concepts/05-advanced-id/concept-01-id-verified.md) - OIDC4IDAの実装ガイド
