...
- チャットサービス（Webhook経由）
- SIEM / 監視基盤（Datadog Logs Intake 等）
...
### Datadog設定（DATADOG_LOG）

[Datadog Logs Intake](https://docs.datadoghq.com/api/latest/logs/#send-logs) へイベントを送信します。**Webhookと同じ `http_request` 統一スキーマ**で設定し、Datadog固有の専用フィールドはありません。`DD-API-KEY` などは汎用の mapping rules で表現します。
      ...
      "execution": {
        ...
        "http_request": {
          ...
          "header_mapping_rules": [
            { "static_value": "${DD_API_KEY}", "to": "DD-API-KEY" }
          ],
          "body_mapping_rules": [
...
| Datadog要求 | 書き場所 | 例 |
|------------|---------|----|
| `DD-API-KEY` ヘッダ | `http_request.header_mapping_rules` | `{ "static_value": "${DD_API_KEY}", "to": "DD-API-KEY" }` |
| `ddsource` / `service` | `http_request.body_mapping_rules` | `{ "static_value": "idp-server", "to": "ddsource" }` |
| `ddtags` 等のイベント値 | `http_request.body_mapping_rules` | `{ "from": "$.type", "to": "ddtags" }` |

> **注意（v0.9以降）**: 旧スキーマでの `DD-API-KEY` / `ddsource` / `ddtags` / `service` の必須バリデーションは撤去されました。これらは利用者が mapping rules で明示的に設定します。`DD-API-KEY` を入れ忘れた場合は Datadog 側が 4xx を返し `HTTP_ERROR` の failure として記録されるため、silent failure にはなりません。

**フィールドの扱い**:
- `Content-Type: application/json` は未指定なら自動付与されるため、ヘッダに書く必要はありません
- `http_request.url` 未設定は `DATADOG_CONFIGURATION_ERROR`（HTTP 呼び出しは行われません）
- `auth_type` / `oauth_authorization` / `retry_configuration` は Webhook と同じ（[Webhook設定](#webhook設定)参照）
