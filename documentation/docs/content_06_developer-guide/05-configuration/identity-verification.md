...
> **利用範囲**: `history.filters` に一致する過去申込みは `$.previous_applications`（配列）として、pre_hook の `verifications` / `additional_parameters` の `condition`、および execution / store / response の mapping_rules から参照できます（[#1592](https://github.com/hirokazu-kobayashi-koba-hiro/idp-server/issues/1592)）。配列射影で各申込みの値を取り出せます（例: `$.previous_applications[*].application_details.external_application_id`）。`$.previous_applications` に現れる範囲は `history.filters` の宣言で決まるため、mapping で使う分だけ filters に宣言してください（過剰露出を避ける）。
...
| `request_json_path` | リクエストから値を取得する JSONPath（例: `$.mobile_phone_number`） |
| `user_claim_json_path` | ユーザー属性から値を取得する JSONPath（例: `$.email` / `$.custom_properties.x[*].id`） |

**JSONPath 表記の注意**:
- どちらのパスも [Jayway JsonPath](https://github.com/json-path/JsonPath) で評価される。**`$` 始まりを推奨**（ネスト・配列 `[*]` を使うには必須）。`"email"` のようなトップレベル単一キーは Jayway が root 解決するため bare でも動くが、表記は `$.email` に揃えると安全。
- **`request_json_path` の root はリクエストボディ直下**（`$.id`）。mapping rules の `$.request_body.*` とは異なるので、`$.request_body.id` と書くと常に不一致＝`400` になる点に注意。
  ...
  "previous_applications": [
    {
      ...
      "type": "申込み種別",
      "status": "申込みステータス",
      "application_details": { "external_application_id": "外部申込みID" }
    }
  ],
  "request_body": {
    "amount": 50000
...
**トークンへの反映に関する注意**:
- `claims:<名前>` スコープによるアクセストークンへの反映では、`status` / `ex_sub` / `roles` / `permissions` / `assigned_tenants` / `assigned_organizations` / `authentication_devices` は**予約クレーム名**であり、User の対応フィールドが優先される。これらの名前で custom_properties を書いてもトークンには反映されない
- 属性更新は**新規の認可**で発行されたトークンに反映される。**リフレッシュグラントでの再発行はグラント保存時のユーザースナップショットを引き継ぐ**ため、承認後の更新は反映されない（UserInfo は常に最新のユーザー属性を返す）
...
**動作**:
- 遷移は `UserLifecycleManager` の遷移ルールに従う（有効な値だが許可されない遷移はエラー）
- 現在と同じステータスへの指定は何もしない（再承認でもエラーにならない）
- **未知のステータス名（typo 等）は KEEP 扱い**: 承認は失敗させず現状ステータスを維持し、誤設定を `error` ログで表面化する（設定不備で承認フロー全体を止めない）

#### applied_user_claims（適用値の記録）

承認時に上記ルール（`user_claims_mapping_rules` / `custom_properties_mapping_rules` / `user_status`）で**実際にユーザーへ適用した値**を、身元確認結果（`identity_verification_result`）の `applied_user_claims` カラムに記録します。設定する項目ではなく、承認時に自動で記録される出力です。

従来は `verified_claims` のみ結果に残り、標準クレーム・custom_properties・ステータスの変更は痕跡が残りませんでした。`applied_user_claims` により、各結果行が「この承認で何が変わったか」を自己記述するため、監査・トラブルシュートに利用できます。
...
- `user_claims`: 実際に適用した標準クレーム（allowlist 適用後の値のみ）
- `custom_properties`: 実際に適用した業務属性
- `user_status`: 実際に遷移したステータス（`KEEP` や同一ステータスで遷移が起きなかった場合は記録されない）
- **適用が無かったパートは省略される**（変わったものだけを保持する）

**参照方法**: 結果取得 API（`GET /{tenant-id}/v1/me/identity-verification/results`）および管理 API のレスポンスに `applied_user_claims` として含まれます。
