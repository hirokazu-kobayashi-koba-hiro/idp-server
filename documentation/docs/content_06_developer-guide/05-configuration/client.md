...
`extension` に `access_token_type` を設定することで、アクセストークンの形式を指定できます。
...
| 値 | 説明 |
|----|------|
| `"JWT"` | JWT形式のアクセストークンを発行 |
| 未設定（デフォルト） | opaque形式のアクセストークンを発行 |

**JWT形式の利点**:
- リソースサーバーがIdPに問い合わせずにトークンを検証可能（自己完結型）
- トークン内にクレーム（スコープ、有効期限等）を含む
...
**パスの命名規則**: `$.{federation-id}.success_count`
