...
- `require_pkce: true`: サーバー側でPKCEを強制（code_challenge無しの認可リクエスト／code_verifier無しのトークンリクエストを拒否）。public client では推奨
- `extension.access_token_duration: 900`: 15分（セキュリティ重視）
- `extension.refresh_token_duration: 3600`: 1時間（短め）

**重要**: `require_pkce: true` を設定すると、code_challenge(S256)/code_verifier の送信が必須になる（未送信は拒否）。詳細は [PKCE実装ガイド](../04-implementation-guides/oauth-oidc/pkce.md)
...
| `access_token_type` | アクセストークンの形式 | `"opaque"` / `"JWT"` |

すべてオプションで、未設定時はTenant設定にフォールバックします。

#### アクセストークンタイプ

アクセストークンの形式（opaque / JWT）は**認可サーバーレベル**の設定（`authorization_server.extension.access_token_type`）でデフォルトを決め、**クライアント単位でオーバーライド**できます（`client.extension.access_token_type`）。未設定時は認可サーバー設定にフォールバックします。
...
認可画面（SPA）のUI表示カスタマイズや、アプリ固有の設定値の伝達に利用します。認証デバイスアプリにも同じ値が届きます。
...
:::warning 秘匿値を入れないこと
認証デバイス向けAPIは認証トランザクションIDのみで到達でき、アクセストークンを要求しません。`context`（`scopes` / `acr_values` 等）と違ってデバイス認証の有無で出し分けもされないため、**IDを知る相手なら誰でも読める前提**で値を設計してください。
:::
