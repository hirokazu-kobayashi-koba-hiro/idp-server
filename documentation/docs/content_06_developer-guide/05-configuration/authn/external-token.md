...
| `$.user` | 認証済みユーザーが確立しているときのみ | 許可リスト投影（`sub` / `email` / `name` / `roles` / `custom_properties` 等）|
...
`$.user` はユーザーが確立していないとキーごと存在しません。「存在しない」ではなく「条件が false 側に倒れる」形で効いてくるため、演算子によって挙動が変わります。

| 条件 | ユーザー未確立（1要素目）| ユーザー確立済み |
|------|:----------------------:|:--------------:|
| `{"operation": "exists", "path": "$.user.sub"}` | スキップ | 実行 |
| `{"operation": "missing", "path": "$.user.sub"}` | 実行 | スキップ |
| `{"operation": "eq", "path": "$.user.email", ...}` | スキップ | 値で判定 |

`missing` を使えば「まだユーザーが確立していないときだけ呼ぶ」も表現できます。
