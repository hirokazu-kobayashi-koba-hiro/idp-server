...
確認コードを送り、本人到達性を検証してから確定する。送信は上記の Email 認証設定
(`email-authentication-challenge` の `execution.details`)をそのまま流用する。

### 認証フローの仕組みには乗せていない

このフローは `AuthenticationTransaction` / `AuthenticationInteractor` を**使わない**。専用テーブル
`email_verification_challenge` に自前で状態を持つ。
...
- 認証ポリシー(`flow` 設定)が不要 — テナントに追加する設定は Email 認証設定とスコープだけ
...
| メソッド・パス | ボディ | スコープ | 説明 |
|---|---|---|---|
| `POST /{tenant-id}/v1/me/email/verification` | なし | `openid` | 現アドレスへコード送信し `{ "id": "<challenge>" }` を返す |
| `POST /{tenant-id}/v1/me/email/verification/{id}/verify` | `{ "verification_code": "..." }` | `openid` | 検証 + `email_verified: true` |
| `POST /{tenant-id}/v1/me/email/change` | `{ "new_email": "..." }` | `email:change` | 新アドレスへコード送信し `{ "id": "<challenge>" }` を返す |
...
- `new_email` は送信前に形式・型検証する(`local@domain.tld` 相当・最大255文字・非文字列は拒否)。不正なら `400` で、メールは送られない
...
操作種別(`verify` / `change`)は**チャレンジ行に永続化**され、必要スコープもそこから導く。URL で
スコープを決めて行で挙動を決める、という食い違いが起きない。変更チャレンジを確認エンドポイントへ
投げても `404` になる。

確定時は `email` / `email_verified` / `preferred_username` の**部分更新**を行う。全カラムを書き戻すと、
呼び出し側が保持する古いユーザー像で `status` 等を巻き戻してしまうため。

### 必要な設定

1. **Email 認証設定**(本ページの `email` config)。`templates` に `email_change` と `email_verify` を
   追加推奨(未定義ならデフォルト文面にフォールバック)。送信設定(`function` / `sender` /
   `sender_config` / `retry_count_limitation` / `expire_seconds`)はそのまま使われる。
2. **`email:change` スコープ**をテナントの `scopes_supported` とクライアントの `scope` に追加する。
