...
- ✅ **Delegation** - actor_token 対応（act クレーム埋め込み + ネスト対応）
...
| K | 交換パターン | Delegation（actor_token） | ✅ | act クレーム埋め込み + ネスト対応 |
...
| `actor_token` | OPTIONAL | アクタートークン（Delegation時） | ✅ |
| `actor_token_type` | CONDITIONAL | actor_token のタイプ識別子（actor_token存在時はREQUIRED） | ✅ |
...
| `invalid_request` | `subject_token` または `subject_token_type` が未指定。未登録の `subject_token_type`。`actor_token` ありで `actor_token_type` なし |
...
## Delegation（actor_token / act クレーム）

### Impersonation vs Delegation

Token Exchange には2つの交換パターンがあります。

**Impersonation（偽装）**: actor_token なし
```
ユーザー → Service A → Token Exchange → 新トークン
                                          sub: ユーザー
                                          （act クレームなし）
→ リソースサーバーからは「ユーザーが直接アクセス」に見える
```

**Delegation（委任）**: actor_token あり
```
ユーザー → Service A → Token Exchange → 新トークン
             │          (actor_token     sub: ユーザー
             │           = Service A)    act: { sub: "service-A" }
             │
→ リソースサーバーは「Service A がユーザーの代理でアクセス」と判別可能
```

### 処理フロー

```
TokenExchangeGrantService.create()
    ↓
┌─────────────────────────────────────────────┐
│ 1. Validator                                │
│    - subject_token, subject_token_type 必須  │
│    - actor_token があれば                    │
│      actor_token_type も REQUIRED            │
└─────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────┐
│ 2. subject_token 検証                        │
│    → SecurityTokenVerificationStrategies    │
│       .get(subjectTokenType)                │
│       .verify(context, subjectToken)        │
└─────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────┐
│ 3. actor_token 検証（Delegation時のみ）      │
│    → SecurityTokenVerificationStrategies    │
│       .get(actorTokenType)                  │
│       .verify(context, actorToken)          │
│    → actor の sub を取得                     │
└─────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────┐
│ 4. ユーザー解決（検索 or JIT Provisioning）  │
└─────────────────────────────────────────────┘
    ↓
┌─────────────────────────────────────────────┐
│ 5. トークン発行                              │
│    → actor 情報を CustomProperties に設定    │
│    → ActorClaimCustomClaimsCreator が        │
│       act クレームを JWT に埋め込み          │
└─────────────────────────────────────────────┘
```

### act クレームの構造（RFC 8693 Section 4.1）
...
### act クレームのネスト（RFC 8693 Section 4.4）

委任チェーン（Service A → Service B → Service C）の場合:
...
最外の `act` が現在の代理者、ネストされた `act` が過去の代理者の履歴。
...
curl -X POST https://api.example.com/{tenant-id}/v1/tokens \
  ...
  -d "actor_token=eyJhbGciOiJFUzI1NiIs..." \
...
| クラス | 役割 |
|--------|------|
| **SecurityToken** | subject_token / actor_token の共通インターフェース |
| **SubjectToken** | SecurityToken 実装。被代理者のトークン |
| **ActorToken** | SecurityToken 実装。代理者のトークン |
| **ActorClaimCustomClaimsCreator** | CustomProperties の actor 情報から act クレームを生成。AccessTokenCustomClaimsCreator プラグインとしてハードコード登録 |
