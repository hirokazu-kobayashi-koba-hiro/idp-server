...
| | `trim` | 空白削除 | なし |
| | `normalize` | Unicode 正規化（NFC/NFD/NFKC/NFKD） | `form` |
...
**用途**: 配列から指定した値に一致する要素を全件削除する（`append` の対。配列属性の登録/解除をペアで構成する）
...
```json
{
  ...
  "functions": [
    {
      "name": "remove",
      "args": {"field": "account_no", "value": "$.application.application_details.remove_account_no"}
    }
  ]
}
```
...
**変換例**:
- スカラー配列: `["finance", "personal", "verified"]` + `value: "personal"` → `["finance", "verified"]`
- オブジェクト配列（field指定）: `[{"account_no": "111"}, {"account_no": "222"}]` + `field: "account_no", value: "222"` → `[{"account_no": "111"}]`

**引数**:
- `value` (必須): 削除対象の値。一致する要素を全件削除する。`$.` で始まる文字列は動的に解決される（後述の「動的引数」参照）
- `field` (任意): 指定時は `element[field]` を抽出して `value` と照合する（オブジェクト配列のキー解除）。未指定時は要素全体を `value` と照合

> **型の注意**: 照合は `Objects.equals` による**厳密な型一致**で行う。`value`（動的解決値）は格納要素の実行時型と一致している必要がある。特に数値配列では型差（例: 格納 `Long` vs 解決 `Integer`、数値配列に文字列 `"1001"` を送る等）で黙ってマッチしない点に注意（文字列ID＝`account_no` 等の用途では問題にならない）。登録側 `merge` のキー照合も同種の型感度のため、register/deregister をペアで使う限り挙動は揃う。

**Null挙動**:
- 入力（`from`）が `null` → `null`（既存配列が無ければ何もしない。空配列を捏造しない）
- `value` が `null` → no-op で配列をそのまま返す（任意リクエストパラメータ欠落時の誤削除を防止。present 時のみ削除したい場合はルールの `condition` で guard）
- `field` 指定で element が Map でない / キー欠落 → 抽出値 null 扱いで非削除（誤削除しない）

**主な使用例**:
- 身元確認の継続フローで、ユーザーの登録済み配列属性から要素を解除（口座解約、サブスク解除等）
- `append` / `merge`（登録）と対にして register / deregister を構成
...
### 14. reshape - オブジェクトのフィールドリネーム/再構成
...
### 15. filter の field 引数 - オブジェクト配列のフィールド条件フィルタリング
...
### 16. normalize - Unicode 正規化

**用途**: 同じ値が表記系違いで届くときに、突合できる形に揃える
...
**変換例**: `"ﾔﾏﾀﾞ ﾀﾛｳ"` → `"ヤマダ タロウ"`

**対応する form**:

| form | 内容 | `"ｶﾞ"`（半角カナ2コードポイント）の結果 |
|------|------|-----------------------------------|
| `NFC` | 正規分解 → 正規合成。表記は保たれる | `"ｶﾞ"`（変化なし） |
| `NFD` | 正規分解のみ | `"ｶﾞ"`（変化なし） |
| `NFKC` | 互換分解 → 正規合成。**既定値** | `"ガ"`（U+30AC、1コードポイント） |
| `NFKD` | 互換分解のみ | `"カ"` + 合成用濁点（U+30AB + U+3099、2コードポイント） |

`form` は大文字小文字を区別しません。未指定・空文字は `NFKC` になります。`NFKX` のような不正な値は例外になります（正規化されないまま素通りすると、この関数に依存した突合が未正規化の値で成立してしまうため）。

:::warning NFKC は表記を畳むので不可逆です
互換分解は「同じ文字の別表記」を1つに寄せます。突合したい場合はこれが目的ですが、**表示用の値に使うと元に戻せません**。
...
表記を保ったまま正規形にしたいだけなら `NFC` を使ってください。
:::

:::danger 同一性判定のフィールドには使わないでください
「表記を畳む」は裏返すと **別の入力が同じ値になる** ということです。`external-api-authentication` の `identity_match_field` のように、**2つの値が一致するかどうかで認証・同一性を判定する**フィールドに適用すると、別人の値が一致し得ます。

外部プロバイダ側で登録できる文字が畳まれて被害者の値に一致する例：

| 攻撃者が登録する値 | NFKC の結果 |
|------|------------|
| `"Ａdmin@example.com"`（先頭が全角Ａ U+FF21） | `"Admin@example.com"` |
| `"ﬁnance@example.com"`（合字 ﬁ U+FB01） | `"finance@example.com"` |

さらに、**見た目では区別できない**畳み込みもあります。ケルビン記号（U+212A）は通常の K（U+004B）に、オングストローム記号（U+212B）は Å（U+00C5）になります。どちらも同じ字形にレンダリングされるので、設定レビューでも入力値の目視でも気づけません。

`case: lower` とチェーンすると畳まれる範囲はさらに広がります。

**`NFC` にすれば安全、ではありません。** 上の U+212A / U+212B は互換分解ではなく**正規分解**を持つため、`NFC` / `NFD` でも畳まれます（互換分解を持つ全角Ａや合字 ﬁ が畳まれるのは NFKC / NFKD のみ）。form の選択はこの問題を解決しません。

`normalize` は**表記ゆれを吸収したい突合**（氏名・住所・表示名など、人間が同じと見なす値の照合）に限定し、同一性判定のキーには生の値を使ってください。
:::

**単機能である**: 空白除去・大文字小文字統一・前後の空白削除は `regex_replace` / `case` / `trim` の担当です。チェーンで組み合わせます。
...
**対象外**:
- **かな⇄カナ変換**: ひらがなとカタカナは表記違いではなく別の文字なので、どの form でも変換されません
- **配列**: 要素ごとに適用する場合は `map` と組み合わせます

```json
{"name": "map", "args": {"function": "normalize", "function_args": {"form": "NFC"}}}
```

引数のキーは `function_args` です。`args` にネストしても `MapFunction` は読まないため、**エラーにならず既定の NFKC が適用されます**。

**主な使用例**:
- 外部API が半角カナ、別経路が全角カナで同じ氏名を返すときの突合（身元確認など）
- 全角/半角が混在する英数字（証券番号、住所等）の突合
- 表示・検索用の正規化された値の生成

いずれも「人間が同じと見なす値を寄せる」用途です。認証・同一性判定のキーには使わないでください（上記 danger）。
...
### パターン4: 配列属性の登録/解除（register / deregister）

ユーザーの登録済み配列属性に対し、リクエスト値で要素を追加（登録）・削除（解除）する。`merge`（登録）と `remove`（解除）を1つのルールにチェーンし、リクエストで `add_accounts` か `remove_account_no` のどちらを送るかで動作を切り替える。`merge` は null source を、`remove` は null value をそれぞれ no-op 扱いするため、片方だけ送れば他方は素通りする。
...
- 登録: `add_accounts` のみ送信 → `merge` が既存に追加（`account_no` で重複排除）、`remove` は no-op
- 解除: `remove_account_no` のみ送信 → `merge` は no-op、`remove` が該当 `account_no` を削除
- 設定例: `config/examples/ekyc/identity-verification-config-array.json`
...
- 入力値の検証を適切に実行
- 機密情報の漏洩防止
- **値を畳む関数は同一性判定のキーに使わない**: `normalize`（NFKC）や `case` のように異なる入力を同じ出力に寄せる関数は、`identity_match_field` のような「2つの値が一致するか」で認証・同一性を判定するフィールドに適用すると、別人の値が一致し得る。表記ゆれの吸収が目的のフィールドに限定する
