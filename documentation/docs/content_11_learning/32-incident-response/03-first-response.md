...
確認する順番:
  ...
  ① ALB メトリクス（外から見た症状）
     ...
     ・レイテンシ → 遅延が発生しているか
     ・ターゲットヘルス → Unhealthy なタスクはあるか

  ② ECS メトリクス（アプリの状態）
     ・Running Task 数 → タスクが死んでないか
     ・CPU / メモリ使用率 → リソース枯渇していないか
     ・タスク再起動回数 → クラッシュループしていないか
...
対応:
  → ロールバックが最速
  → Blue-Green なら ALB の重みを Blue に戻すだけ
...
```
状況: Aurora への接続がエラーになっている

確認:
  □ Aurora のステータス（AWS コンソール）
  □ コネクションプールの使用率
  □ DB の CPU / メモリ
...
対応:
  ...
  → Aurora 障害: AWS のステータスページ確認、サポートケース起票
