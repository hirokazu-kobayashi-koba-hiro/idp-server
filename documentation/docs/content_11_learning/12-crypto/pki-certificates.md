...
上の「検証の流れ」はリンクの署名とトラストストア到達を見ているが、**それだけではチェーンは成立しない**。

```
なぜ足りないか:

  鍵は、署名する相手を選ばない。
  署名鍵はどんなデータにでも署名できる。証明書もデータなので、
  末端（エンドエンティティ）の証明書の鍵でも、別の証明書を作れてしまう。

┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  攻撃者が持っているもの                                       │
│    自分の鍵 K1 と、それに対する正規のチェーン                 │
│      [ L1(K1) , 中間CA , ルートCA ]                          │
│         ↑ L1 は末端の証明書（CA ではない）                    │
│                                                             │
│  攻撃者がやること                                             │
│    K1 で、別の鍵 K0 を subject とする証明書 L0 に署名         │
│                                                             │
│  提示するチェーン                                             │
│      [ L0(K0) , L1(K1) , 中間CA , ルートCA ]                 │
│         ↑ 偽物   ↑ L1 が「発行者」の位置にいる                │
│                                                             │
│  このチェーンは                                               │
│    ✓ 全リンクの署名検証を通る                                 │
...
RFC 5280 は、発行する権限を証明書自身に書かせる。**発行者の位置に現れる証明書に、これらを要求する**のが上を止める箇所になる。
...
│                            │ 他の証明書を発行してよい        │
├─────────────────────────────────────────────────────────────┤
│ Basic Constraints           │ 自分より下に許す CA の段数      │
│   pathLenConstraint        │ リーフは CA ではないので数えない │
├─────────────────────────────────────────────────────────────┤
...
pathLenConstraint の数え方:
  ...
  I が leaf を発行  → I より下の CA は 0 個  → I は pathLen >= 0
  root が I を発行  → root より下の CA は 1 個（I）→ root は pathLen >= 1
```

先の例では `L1` に `cA=TRUE` が無いため、`L1` が発行者の位置に現れた時点で拒否される。
...
1. パース
2. 有効期限
3. 発行者の資格        ← cA / pathLen / keyCertSign
4. リンク署名
5. トラストストア到達
──────────────────────
6. ここで初めて証明書の中身（SAN、拡張領域）を判断に使う
```

**3 と 4 は別の検査**で、どちらか片方では意味を持たない。

また 6 が最後なのは、証明書の拡張領域が提示側の自由に書ける場所だから。信頼点まで遡れる前に読むと、まだ信頼していないデータで分岐することになる。

:::tip 実装では
Java の `CertPathValidator`（PKIX）はこれらを含めて検証する。自前でチェーンを辿る場合は
`X509Certificate#getBasicConstraints()`（CA でなければ -1、CA なら pathLenConstraint）と
`getKeyUsage()[5]`（keyCertSign）を自分で確認する必要がある。
:::
