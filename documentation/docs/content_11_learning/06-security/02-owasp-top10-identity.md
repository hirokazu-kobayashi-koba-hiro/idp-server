...
DNSリバインディングは、DNS応答を操作してSSRF保護をバイパスする攻撃手法です。

```
攻撃の流れ:
1. 攻撃者は自身のドメイン（evil.com）のDNS TTLを極端に短く設定（例: 1秒）
2. 被害者アプリがevil.comを検証 → 最初のDNS解決で外部IP（203.0.113.1）を返す
3. アプリは「外部IPなので安全」と判断して許可
4. 実際の接続時にDNSを再解決 → 今度は内部IP（169.254.169.254）を返す
5. アプリは内部リソース（クラウドメタデータ等）にアクセスしてしまう

対策:
- DNS解決は1回のみ行い、解決したIPアドレスを保持して再利用
- 接続前にIPアドレスを再検証（解決後のIPがプライベートIPでないか確認）
- DNSキャッシュのTTLに最小値を設定（攻撃者の短いTTLを無効化）
```
