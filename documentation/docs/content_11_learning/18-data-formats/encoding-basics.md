...
結果として、ASCII 入力で `+` `/` を生むのは次の文字だけになります。
...
しかも**位置が限られます**。4つ目の塊は3バイト目の下位6ビットなので、該当するのは
先頭から数えて **3 バイトごとの区切りの最後**、つまり 0 始まりで **2・5・8…番目**
（1 始まりなら 3・6・9…番目）に来たときだけです。

```
index : 0  1  2   3  4  5   6  7  8
        a  a  >   a  a  a   a  a  a   → YWE+YWFhYWFh   + が出る
        a  >  a   a  a  a   a  a  a   → YT5hYWFhYWFh   出ない
```

DEL は制御文字なので実際の識別子やパスワードには通常現れません。**実務で意識するのは
`>` `?` `~` の3文字**です。

非 ASCII（UTF-8 のマルチバイト）を含む場合は、バイト値が 0x80 以上になるため
4 つ目以外の塊でも 62・63 に到達しえます。条件はもっと緩くなります。
