...
| 認証器 | ルート CA | 入手先 |
|-------|----------|-------|
| YubiKey | Yubico Root CA | [Yubico Developers](https://developers.yubico.com/U2F/Attestation_Certificates/) |
| Windows Hello | Microsoft TPM Root | Windows Update / Microsoft |
| Android | Google Hardware Attestation Root | [Google](https://developer.android.com/training/articles/security-key-attestation) |
| Apple | Apple WebAuthn Root CA | [Apple PKI](https://www.apple.com/certificateauthority/) |
| 各種 FIDO 認定認証器 | FIDO MDS | [FIDO Alliance MDS](https://fidoalliance.org/metadata/) |

### FIDO Metadata Service (MDS) の活用

> 📖 MDS の詳細については [AAGUID - 認証器の識別](./fido2-aaguid-authenticator-identification.md#fido-metadata-service-mds) を参照してください。
...
> 📖 商用環境での運用については [商用環境でのアテステーション検証](./fido2-attestation-production-considerations.md) を参照してください。
