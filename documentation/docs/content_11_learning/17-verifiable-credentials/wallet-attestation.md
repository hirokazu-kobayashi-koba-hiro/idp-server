...
Issuer が Wallet Provider を信頼する根拠は、欧州委員会が公開する一覧（**LoTE**）です。認定を受けた Provider が加盟国から通知されて載り、Issuer はその一覧と照合します（[EUDI Wallet の全体像](./eu-wallet-ecosystem.md)）。
...
## 実装例: IT-Wallet の WIA 発行フロー

イタリアの IT-Wallet は、この空白を具体的な手順で埋めています。インストール直後にハードウェア鍵を Wallet Provider に登録（初期化）したうえで、WIA を次の手順で発行します。有効化の全体（初期化から、国の IdP でログインして PID を受け取るまで）は [IT-Wallet の生態系](./it-wallet-ecosystem.md) を参照してください。
...
:::note ロック解除とポータルへのログインは別に定められている
IT-Wallet はウォレットのロック解除に PIN または生体認証を設定させます。仕様上の用途は、**アプリを開くことと、操作を承認すること**です。

Wallet Provider の Web ポータルへのログインは、これとは別に **2 要素以上の認証**が求められています。ポータルからは、端末が無くても Wallet Instance の失効を頼めます。
:::

利用者から見た有効化の順番は、仕様の機能要件が定めています。PIN を設定し、**国の IdP で認証して PID を受け取る**ところまでが有効化です。国の IdP への認証は PID Provider の発行フローの中で行われ、Wallet Provider への登録とは別の段階です。

一方、Wallet Provider のアカウントに対して利用者をどう認証するかは、Wallet Provider の裁量に残されています。
...
| 論点 | 設計の答え |
|---|---|
| Issuer はどうやってウォレットを信頼するか | Wallet Provider を LoTE で信頼し、Provider が個々のインスタンスを保証する |
