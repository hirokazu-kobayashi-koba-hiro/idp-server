...
public class CredentialController {
    ...
    private final JWTVerifier proofVerifier;
    ...
    public ResponseEntity<?> issueCredential(
        ...
        TokenInfo tokenInfo = tokenService.validate(accessToken);

        // Proof の検証
        if (request.getProof() != null) {
            verifyProof(request.getProof(), tokenInfo.getCNonce());
        }

        // Credential の発行
        CredentialResult result = issuerService.issue(
            tokenInfo.getSubject(),
            request.getFormat(),
            request.getCredentialDefinition()
        );

        // 即時発行 or 遅延発行
        if (result.isImmediate()) {
            return ResponseEntity.ok(CredentialResponse.builder()
    ...
    private void verifyProof(Proof proof, String expectedNonce) {
        if ("jwt".equals(proof.getProofType())) {
            ...
            if (!expectedNonce.equals(claims.getStringClaim("nonce"))) {
                throw new InvalidProofException("Invalid nonce");
            }

            // 署名の検証
            JWK jwk = JWK.parse(jwt.getHeader().getJWK().toJSONObject());
...
public class CredentialIssuerService {
    ...
    public CredentialResult issue(String subject, String format,
        ...
        vc.put("type", definition.getType());
        vc.put("issuer", issuerDid);
        vc.put("issuanceDate", Instant.now().toString());
        vc.put("credentialSubject", buildCredentialSubject(subject));
        ...
        jwt.sign(new ECDSASigner((ECPrivateKey) issuerPrivateKey));

        return CredentialResult.immediate(jwt.serialize());
    }
}
```
...
class CredentialWallet {
  ...
  // Credential Offer を処理
  async handleCredentialOffer(offerUri) {
    // Offer を取得
    const offer = await this.fetchCredentialOffer(offerUri);
    ...
    const authMetadata = await this.fetchAuthServerMetadata(authServer);

    // グラント種別に応じてフローを選択
    if (offer.grants['urn:ietf:params:oauth:grant-type:pre-authorized_code']) {
      return await this.preAuthorizedCodeFlow(offer, metadata, authMetadata);
    } else {
      return await this.authorizationCodeFlow(offer, metadata, authMetadata);
    }
  }

  async preAuthorizedCodeFlow(offer, issuerMetadata, authMetadata) {
    const grant = offer.grants['urn:ietf:params:oauth:grant-type:pre-authorized_code'];

    // TX コードが必要な場合はユーザー入力を取得
    let txCode = null;
    if (grant.tx_code) {
      txCode = await this.promptUserForTxCode(grant.tx_code);
    }
    ...
    // Credential を取得
    return await this.requestCredential(
  ...
  async requestCredential(credentialEndpoint, accessToken, nonce,
                          configId, issuerMetadata) {
    const config = issuerMetadata.credential_configurations_supported[configId];

    // Proof of Possession を生成
    ...
    const response = await fetch(credentialEndpoint, {
      ...
      body: JSON.stringify({
        ...
        credential_definition: config.credential_definition,
        proof: {
          proof_type: 'jwt',
          jwt: proof
        }
  ...
  async createProof(audience, nonce) {
    const header = {
      alg: 'ES256',
      typ: 'openid4vci-proof+jwt',
      kid: `${this.did}#key-1`
...
const wallet = new CredentialWallet({
  ...
  did: 'did:key:z6Mkj3PUd1WjvaDhNZhhhXQdz5UnZXmS7ehtx8bsPpD47kKc'
});

// QR コードから Credential Offer を読み取り
const offerUri = 'https://issuer.example.com/offers/abc123';
const credential = await wallet.handleCredentialOffer(offerUri);
await wallet.storeCredential(credential.credential, {
  format: 'jwt_vc_json',
  issuer: 'https://issuer.example.com',
  display: { name: '大学学位証明書' }
});
```
