...
```java
@RestController
@RequestMapping("/register/{clientId}")
public class ClientManagementController {

    @GetMapping
    public ResponseEntity<ClientInfo> getClient(
            @PathVariable String clientId,
            @RequestHeader("Authorization") String authorization) {

        // Registration Access Token の検証
        String token = extractBearerToken(authorization);
        validateRegistrationAccessToken(token, clientId);

        // クライアント情報の取得
        RegisteredClient client = clientRepository.findByClientId(clientId)
            .orElseThrow(() -> new ClientNotFoundException(clientId));

        return ResponseEntity.ok(toClientInfo(client));
    }

    @PutMapping
    public ResponseEntity<ClientInfo> updateClient(
            @PathVariable String clientId,
            @RequestHeader("Authorization") String authorization,
        ...
        if (!clientId.equals(request.getClientId())) {
            throw new InvalidClientMetadataException("client_id mismatch");
        }

        // メタデータの検証
        validateClientMetadata(request);

        // クライアントの更新
        RegisteredClient updated = updateClient(clientId, request);

        // 新しい Registration Access Token の発行（オプション）
        String newToken = rotateRegistrationAccessToken(clientId);

        return ResponseEntity.ok(toClientInfo(updated, newToken));
    }
    ...
    public ResponseEntity<Void> deleteClient(
        ...
        // 関連するトークンの無効化
        tokenRepository.revokeByClientId(clientId);
...
class ClientRegistrationManager {
  constructor(registrationClientUri, registrationAccessToken) {
    this.uri = registrationClientUri;
    this.token = registrationAccessToken;
  }
  ...
  async getClientInfo() {
    ...
    if (!response.ok) {
      throw new Error(`Failed to get client info: ${response.status}`);
    }

    const clientInfo = await response.json();
    // トークンが更新されている場合は保存
    if (clientInfo.registration_access_token !== this.token) {
      this.token = clientInfo.registration_access_token;
  ...
  async updateClient(updates) {
    // 現在の設定を取得
    const current = await this.getClientInfo();

    // 更新をマージ（完全置換なので全フィールドを送信）
    const updated = {
      ...current,
      ...updates,
      // 読み取り専用フィールドを除外
      client_id_issued_at: undefined,
      registration_access_token: undefined,
      registration_client_uri: undefined
    };
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update client: ${error.error_description}`);
    }

    const result = await response.json();
    // トークンが更新されている場合は保存
    if (result.registration_access_token !== this.token) {
      this.token = result.registration_access_token;
      await this.saveToken(this.token);
    }
...
// 使用例
const manager = new ClientRegistrationManager(
  'https://auth.example.com/register/s6BhdRkqt3',
  'reg-token-abc123'
);

// redirect_uri の追加
await manager.updateClient({
  redirect_uris: [
    'https://client.example.com/callback',
    'https://client.example.com/callback2'
  ]
