...
```java
@PostMapping("/par")
public ResponseEntity<PARResponse> pushedAuthorizationRequest(
        @RequestBody PARRequest request) {

    // authorization_details のパースと検証
    List<AuthorizationDetail> details = parseAuthorizationDetails(
        request.getAuthorizationDetails()
    );

    for (AuthorizationDetail detail : details) {
        // type ごとのバリデーション
        AuthorizationDetailValidator validator =
            validatorRegistry.getValidator(detail.getType());
        validator.validate(detail);
    }

    // PAR を保存
    String requestUri = generateRequestUri();
    parRepository.save(requestUri, request, details);
...
public ResponseEntity<TokenResponse> token(
    ...
    // 認可コードから authorization_details を取得
    AuthorizationCode code = validateAuthorizationCode(request.getCode());
    List<AuthorizationDetail> details = code.getAuthorizationDetails();

    // JWT アクセストークンを生成
    String accessToken = jwtBuilder
        .subject(code.getSubject())
        .audience(extractAudience(details))
        .claim("authorization_details", details)
...
class RARClient {
        ...
        authorization_details: [
          {
            ...
            creditorAccount: {
              iban: paymentDetails.creditorIban
            },
            creditorName: paymentDetails.creditorName,
            remittanceInformationUnstructured: paymentDetails.reference
          }
        ]
      })
    });
  ...
  async handleCallback(code) {
    ...
    // authorization_details を確認
    console.log('Granted authorization:', tokens.authorization_details);
...
```java
public void validateRequest(String accessToken, PaymentRequest request) {
    JWT jwt = JWT.parse(accessToken);

    // authorization_details の検証
    List<Map<String, Object>> details = jwt.getClaim("authorization_details");

    PaymentAuthorizationDetail paymentAuth = findPaymentAuthorization(details);
    if (paymentAuth == null) {
        throw new InsufficientAuthorizationException("No payment authorization");
    }

    // 金額の検証
    if (request.getAmount().compareTo(paymentAuth.getInstructedAmount()) > 0) {
        throw new InsufficientAuthorizationException("Amount exceeds authorized limit");
    }

    // 送金先の検証
    if (!request.getCreditorIban().equals(paymentAuth.getCreditorIban())) {
        throw new InsufficientAuthorizationException("Creditor account mismatch");
    }
}
```
