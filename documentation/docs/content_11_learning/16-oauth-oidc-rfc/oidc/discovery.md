...
public class OIDCDiscoveryClient {
    ...
    public OIDCProviderMetadata discover(String issuer) throws Exception {
        ...
        if (response.statusCode() != 200) {
            throw new DiscoveryException("Failed to fetch metadata: " + response.statusCode());
        }

        // パースと検証
        OIDCProviderMetadata metadata = objectMapper.readValue(
            response.body(), OIDCProviderMetadata.class
        );

        // issuer の検証
        if (!issuer.equals(metadata.getIssuer())) {
            throw new DiscoveryException("Issuer mismatch");
        }

        return metadata;
    }

    public JWKSet fetchJWKS(OIDCProviderMetadata metadata) throws Exception {
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(metadata.getJwksUri()))
            .GET()
            .build();
...
OIDCProviderMetadata metadata = discoveryClient.discover("https://auth.example.com");

// エンドポイントを取得
String authEndpoint = metadata.getAuthorizationEndpoint();
String tokenEndpoint = metadata.getTokenEndpoint();

// サポートされている機能を確認
if (metadata.getCodeChallengeMethodsSupported().contains("S256")) {
    // PKCE S256 を使用
}

// JWKS を取得
JWKSet jwks = discoveryClient.fetchJWKS(metadata);
```
...
class OIDCClient {
  ...
  async discover() {
    ...
    // issuer の検証
    if (this.metadata.issuer !== this.issuer) {
      throw new Error(`Issuer mismatch: expected ${this.issuer}, got ${this.metadata.issuer}`);
    }
  ...
  async fetchJWKS() {
    ...
    const response = await fetch(this.metadata.jwks_uri);
    if (!response.ok) {
  ...
  supportsFeature(feature) {
    switch (feature) {
      case 'pkce':
        return this.metadata?.code_challenge_methods_supported?.includes('S256');
      case 'par':
        return !!this.metadata?.pushed_authorization_request_endpoint;
      case 'dpop':
        return !!this.metadata?.dpop_signing_alg_values_supported;
      case 'backchannel_logout':
        return this.metadata?.backchannel_logout_supported === true;
      default:
        return false;
    }
  }
  ...
  getAuthorizationUrl(options) {
    if (!this.metadata) {
      throw new Error('Metadata not loaded. Call discover() first.');
    }
    ...
    // PKCE を追加（サポートされている場合）
    if (this.supportsFeature('pkce') && options.codeChallenge) {
      params.set('code_challenge', options.codeChallenge);
      params.set('code_challenge_method', 'S256');
    }

    return `${this.metadata.authorization_endpoint}?${params}`;
  }
}

// 使用例
const client = new OIDCClient('https://auth.example.com');

await client.discover();

console.log('Authorization Endpoint:', client.metadata.authorization_endpoint);
console.log('PKCE supported:', client.supportsFeature('pkce'));
console.log('PAR supported:', client.supportsFeature('par'));
