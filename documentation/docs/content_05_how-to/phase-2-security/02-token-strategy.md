...
認可サーバーのデフォルトを、クライアント単位で上書きできます。クライアント設定の `extension` で指定し、未設定時は認可サーバー設定にフォールバックします。
...
これにより、同一テナント内で「Web APIクライアントはJWT（ローカル検証重視）／管理画面クライアントはopaque（即時失効重視）」のように形式を使い分けられます。

### デフォルト設定

設定を省略した場合、**opaque** が適用されます。即時失効が可能なセキュアなデフォルトです。

> **注意**: クライアント設定（`client.extension.access_token_type`）が未指定の場合は認可サーバーレベルの設定が適用されます。
