...
| フィールド                  | 型      | 説明                                                                      |
|------------------------|--------|-------------------------------------------------------------------------|
| `operation`            | string | 比較演算子（省略時 `eq`）。`in`/`nin`/`contains` 等で配列メンバーシップ等を表現。詳細は[設定リファレンス](../../../content_06_developer-guide/05-configuration/identity-verification.md) |
| `request_json_path`    | string | リクエストから値を取得する JSONPath。root は**リクエストボディ直下**（`$.id`）。mapping rules の `$.request_body.*` とは異なる |
| `user_claim_json_path` | string | ユーザー属性から値を取得する JSONPath（`$` 始まり推奨。`[*]` で配列取得）　[JsonPath](https://github.com/json-path/JsonPath) |
...
| `previous_applications` | `history.filters` に一致する過去の申込みリスト（各要素は `application` と同形・`[*]` 射影可） |
