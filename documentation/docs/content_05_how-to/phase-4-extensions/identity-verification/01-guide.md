...
### 🧪 例6：過去の申込みを配列で参照（`previous_applications`）

```json
{
  "from": "$.previous_applications[*].application_details.external_application_id",
  "to": "past_external_application_ids"
}
```

→ `history.filters` に一致する過去申込みすべてから外部申込みIDを取り出して、**配列**で渡す！（`[*]` で各要素を射影）

`[*]` の後ろを省いて `$.previous_applications[*].application_details` にすれば、各 `application_details` を**オブジェクトの配列**として渡せるよ。

> 注意: `$.previous_applications` に現れる範囲は `history.filters` の宣言次第。mapping で使う分だけ filters に宣言してね。
...
| `previous_applications` | `history.filters` に一致する過去の申込みリスト（各要素は `application` と同形。`[*]` で配列射影可） |
