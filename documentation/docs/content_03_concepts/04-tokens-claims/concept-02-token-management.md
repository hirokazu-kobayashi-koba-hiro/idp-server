...
自己完結型検証では、リソースサーバーは署名を確かめるだけでは足りません。**そのトークンが自分宛てか**を確認する必要があります。署名が正しくても、別のリソースサーバー向けに発行されたトークンを受け入れてしまえば、片方に渡したトークンでもう片方が使えることになるためです。

JWTアクセストークンの`aud`はこの宛先を表し、**リソースサーバーの識別子**が入ります。クライアントではありません（クライアントは`client_id`クレームが表します）。IDトークンの`aud`がクライアントを指すのとは役割が逆になります。

idp-serverはスコープからリソースを決めます。「このスコープはこのAPIのもの」という対応をテナントに設定し、付与されたスコープに応じて`aud`を決定します。対応づけていないテナントでは、issuerが入ります。

複数のリソースにまたがるスコープ要求は`invalid_scope`で拒否されます。1つのトークンが複数のリソースを名乗ると、それぞれが他所向けのスコープを受け入れることになるためです。

設定方法は[リソース単位のアクセストークン](../../content_05_how-to/phase-2-security/06-resource-indicator.md)を参照してください。

## 関連ドキュメント

- [トークン有効期限パターン](../../content_05_how-to/phase-2-security/02-token-strategy.md) - 具体的な設定例
- [リソース単位のアクセストークン](../../content_05_how-to/phase-2-security/06-resource-indicator.md) - `aud`の設定
