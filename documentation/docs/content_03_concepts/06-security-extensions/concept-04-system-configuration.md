...
| `trusted_headers` | 信頼するフォワーディングヘッダーの宣言（**現状は情報的な設定**。ゲート判定は `enabled` + `addresses` のみで行われ、ヘッダー単位の参照は未実装） | `["X-Forwarded-For", "X-Forwarded-Proto", "X-Forwarded-Host", "X-Real-IP"]` |

:::warning `trusted_proxies` が守る範囲

`trusted_proxies` がゲートするのは **`X-Forwarded-For` による「クライアントIP解決」のみ**です（監査ログ・セキュリティイベント等で使う実クライアントIPの取得）。

**DPoP の htu 検証は対象外**です。DPoP（RFC 9449）の `htu` クレーム検証で使うクライアント向け URL は `X-Forwarded-Proto` / `X-Forwarded-Host` から再構築されますが、これらは**受信値をそのまま使用**します。その完全性は「idp-server が信頼イングレス（リバースプロキシ / API Gateway + ALB）経由でのみ到達可能」という**ネットワーク隔離**（private subnet + security group 等）に依存し、`trusted_proxies` 設定では守られません。`trusted_proxies` を設定すれば htu も保護される、という理解は誤りです。
:::

> DPoP の htu を成立させるための `X-Forwarded-Proto` / `X-Forwarded-Host` 転送設定（nginx / AWS API Gateway）は [How-to: システムセキュリティ設定 - リバースプロキシ / API Gateway のヘッダー転送](../../content_05_how-to/phase-2-security/04-system-security-setup.md#リバースプロキシ--api-gateway-のヘッダー転送dpop-利用時) を参照してください。
