...
```mermaid
graph TB
    subgraph Client["クライアント側（ユーザーのデバイス）"]
        Browser["Webブラウザー<br/>(User Agent)<br/>Chrome, Safari, Edge等"]
        Authenticator["認証器<br/>(Authenticator)<br/>Touch ID, YubiKey等"]
    end

    subgraph Server["サーバー側（サービス提供者）"]
        RP["RP<br/>(Relying Party)<br/>Webサービス<br/>例: Google, GitHub"]
    end

    User["👤 ユーザー"] -->|1. 操作| Browser
    Browser -->|2. WebAuthn API| Authenticator
    Authenticator -->|3. 生体認証/PIN| User
    Authenticator -->|4. 公開鍵・署名| Browser
    Browser -->|5. HTTP通信| RP
    RP -->|6. 検証結果| Browser
    Browser -->|7. 結果表示| User
...
sequenceDiagram
    ...
    participant Browser as Webブラウザー<br/>(User Agent)
    participant RP as RP<br/>(Webサーバー)
    participant Auth as 認証器<br/>(Authenticator)

    User->>Browser: 1. "パスキー登録"クリック
    Browser->>RP: 2. チャレンジリクエスト
    RP->>RP: 3. チャレンジ生成（32バイト）
    RP-->>Browser: 4. チャレンジ + RP情報返却
    Browser->>Auth: 5. 鍵ペア生成要求
    Auth->>User: 6. 生体認証要求
    User->>Auth: 7. 生体認証実行
    Auth->>Auth: 8. 鍵ペア生成
    Auth-->>Browser: 9. 公開鍵・Attestation返却
    Browser->>RP: 10. 公開鍵送信
    RP->>RP: 11. Origin/Challenge/署名検証
    RP->>RP: 12. 公開鍵をDB保存
    RP-->>Browser: 13. 登録成功
    Browser-->>User: 14. "登録完了"表示
```
...
sequenceDiagram
    ...
    User->>Browser: 1. "ログイン"クリック
    Browser->>RP: 2. チャレンジリクエスト
    RP->>RP: 3. チャレンジ生成
    RP->>RP: 4. Credential ID取得（DB）
    RP-->>Browser: 5. チャレンジ + Credential ID返却
    Browser->>Auth: 6. 認証要求
    Auth->>User: 7. 生体認証要求
    User->>Auth: 8. 生体認証実行
    Auth->>Auth: 9. 秘密鍵で署名生成
    Auth-->>Browser: 10. 署名データ返却
    Browser->>RP: 11. 署名送信
    RP->>RP: 12. 公開鍵取得（DB）
    RP->>RP: 13. Origin/Challenge/署名検証
    RP->>RP: 14. signCount検証（クローン検出）
    RP-->>Browser: 15. 認証成功
