...
Management APIでセキュリティイベントを検索する際、`user_name`パラメータでユーザー名による部分一致検索が可能です：

```
GET /v1/management/tenants/{tenant-id}/security-events?user_name=john
```

これにより、特定ユーザーに関連するイベントを効率的に絞り込むことができます。
