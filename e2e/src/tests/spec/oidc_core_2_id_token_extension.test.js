  ...
  describe("2.  ID Token", () => {
    it("iss REQUIRED. Issuer Identifier for the Issuer of the response. The iss value is a case sensitive URL using the https scheme that contains scheme, host, and optionally, port number and path components and no query or fragment components.", async () => {
      const { authorizationResponse } = await requestAuthorizations({
        ...
        state: "aiueo",
        scope: "openid profile phone email claims:status claims:authentication_devices claims:roles claims:permissions claims:assigned_tenants claims:assigned_organizations claims:authentication_devices " + clientSecretPostClient.scope,
