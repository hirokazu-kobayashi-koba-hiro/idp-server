...
describe("User Status verification. Can not authorize when user status is not active.", () => {
   ...
   */
  const changeStaus = async ({organizationId, tenantId, adminAccessToken, user, status}) => {


    return await patchWithJson({
      url: `${backendUrl}/management/organizations/${organizationId}/${tenantId}/v1/users/${user.sub}`,
      headers: {
  ...
  describe("status verification", () => {
    ...
    it("Status lock is can not authorize", async () => {
      ...
      console.log("Step 6: Changing Status...");
      let status = "LOCKED";
      ...
      const { authorizationResponse: lockedAuthorizationResponse } = await requestAuthorizations({
        endpoint: `${backendUrl}/${tenantId}/v1/authorizations`,
        authorizeEndpoint: `${backendUrl}/${tenantId}/v1/authorizations/{id}/authorize`,
        denyEndpoint: `${backendUrl}/${tenantId}/v1/authorizations/{id}/deny`,
        clientId: clientId,
      ...
      console.log(lockedAuthorizationResponse);
      expect(lockedAuthorizationResponse.code).toBeUndefined();
