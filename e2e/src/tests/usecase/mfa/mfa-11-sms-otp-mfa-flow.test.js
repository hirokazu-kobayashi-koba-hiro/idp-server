...
/**
 * MFA Use Case: SMS OTP + Password MFA Flow
 *
 * verify.sh (mfa-sms) の Phase 1-2 を E2E Jest に移植。
 *
 * カバー範囲 (GAP-ANALYSIS A-06):
 * - Phase 1: ユーザー登録 (initial-registration → authorize → token)
 * - Phase 2: MFA ログイン (Password 1st → SMS OTP challenge → verification → token)
 * - amr (Authentication Methods References) に sms + password が含まれること
 * - UserInfo / Refresh Token 動作確認
 */
describe("MFA Use Case: SMS OTP + Password MFA Flow", () => {
  ...
  beforeAll(async () => {
    ...
    const adminEmail = `admin-${timestamp}@mfa-sms.example.com`;
    ...
    const onboardingResponse = await onboarding({
      body: {
        organization: {
          id: organizationId,
          name: `MFA SMS Test Org ${timestamp}`,
          description: "E2E test for SMS OTP MFA flow",
        },
        tenant: {
          id: tenantId,
          name: `MFA SMS Test Tenant ${timestamp}`,
          domain: backendUrl,
          ...
          session_config: {
            cookie_name: `MFA_SMS_${organizationId.substring(0, 8)}`,
        ...
        client: {
          ...
          client_name: "MFA SMS Test Client",
    ...
    // Register SMS authentication config (http_request to mock server)
    const smsAuthResp = await postWithJson({
      ...
      body: {
        ...
        metadata: {
          type: "external",
          description: "SMS authentication via mock",
    ...
    testEmail = `sms-user-${timestamp}@mfa-sms.example.com`;
    testPassword = "SmsTestPass_1!";
  ...
  it("should complete MFA login with Password (1st factor) + SMS OTP (2nd factor) and verify amr", async () => {
    console.log("\n=== Phase 2: MFA Login (Password + SMS OTP) ===");

    // Step 1: Start new authorization flow
    console.log("Step 1: Start authorization flow for MFA login");
    const state = `mfa-sms-${Date.now()}`;
    ...
    console.log("Password authentication successful (1st factor complete)");

    // Step 3: SMS OTP Challenge (2nd factor)
    console.log("\nStep 3: SMS OTP Challenge");
    ...
    console.log("SMS OTP verified (2nd factor complete)");
    ...
    // amr should contain both "sms" and "password"
    ...
    expect(userinfoResp.data.name).toBe("MFA SMS Test User");
    console.log(
    ...
    console.log("\n=== MFA SMS OTP Flow Test Completed ===");
  });
});
