...
/**
 * /v1/me Use Case: self-service email verification and change (Issue #1416).
 *
 *   POST /v1/me/email/verification            (scope openid)       -> code to the CURRENT address
 *   POST /v1/me/email/verification/{id}/verify (scope openid)       -> email_verified: true
 *   POST /v1/me/email/change      {new_email}  (scope email:change) -> code to the NEW address
 *   POST /v1/me/email/change/{id}/verify       (scope email:change) -> commits the new email
 *
 * Two endpoints because the operations carry different privilege: verification only sets a claim,
 * a change moves preferred_username — the login identifier — under an EMAIL identity policy.
 *
...
const changeScope = "openid email email:change";

/**
 * idp-server generates the code and never returns it in an API response — returning it would defeat
 * the point, which is proving the address is reachable. So the test observes the outbound mail: the
 * tenant's sender POSTs to a mockoon CRUD route, and we read the newest entry for that recipient.
 */
async function readSentCode(targetEmail) {
  const resp = await get({ url: "http://localhost:4000/sent-emails" });
  expect(resp.status).toBe(200);
  const sent = resp.data.filter((entry) => entry.to === targetEmail);
  expect(sent.length).toBeGreaterThan(0);
  const body = sent[sent.length - 1].body;
  const matched = /(\d{6})/.exec(body);
...
async function startChange(ctx, accessToken, newEmail) {
  ...
  const challengeId = startResp.data.id;
  return { challengeId, code: await readSentCode(newEmail) };
}

function submitChangeCode(ctx, accessToken, challengeId, code) {
  return postWithJson({
    url: `${backendUrl}/${ctx.tenantId}/v1/me/email/change/${challengeId}/verify`,
    headers: createBearerHeader(accessToken),
...
async function runEmailChange(ctx, accessToken, newEmail) {
  const { challengeId, code } = await startChange(ctx, accessToken, newEmail);
  return submitChangeCode(ctx, accessToken, challengeId, code);
}

async function runEmailVerification(ctx, accessToken, currentEmail) {
  const startResp = await postWithJson({
  ...
  expect(startResp.status).toBe(200);
  const challengeId = startResp.data.id;
  const code = await readSentCode(currentEmail);

  return postWithJson({
    url: `${backendUrl}/${ctx.tenantId}/v1/me/email/verification/${challengeId}/verify`,
    headers: createBearerHeader(accessToken),
    body: { verification_code: code },
  });
}

describe("Me Use Case: self-service email verification and change", () => {
  ...
  it("verification endpoint verifies the current address without changing it", async () => {
    ...
    const verifyResp = await runEmailVerification(ctx, token, email);
    expect(verifyResp.status).toBe(200);
  ...
  it("verification endpoint ignores a caller-supplied new_email", async () => {
    ...
    const startResp = await postWithJson({
      ...
      body: { new_email: `hijack-${Date.now()}@me-email.example.com` },
    });
    expect(startResp.status).toBe(200);
    const code = await readSentCode(ctx.adminEmail);
  ...
  it("security: a token without email:change cannot start or finish a change", async () => {
    ...
    const scopedToken = (await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword, changeScope))
      .data.access_token;
    const started = await startChange(ctx, scopedToken, `scope-${Date.now()}@me-email.example.com`);
    ...
    const startDenied = await postWithJson({
      ...
      body: { new_email: `denied-${Date.now()}@me-email.example.com` },
    });
  ...
  it("security: a change challenge cannot be committed through the verification endpoint", async () => {
    ...
    const token = (await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword, changeScope)).data
      .access_token;
    const started = await startChange(ctx, token, `cross-${Date.now()}@me-email.example.com`);
    ...
    expect((await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword)).status).toBe(200);
    const proper = await submitChangeCode(ctx, token, started.challengeId, started.code);
  ...
  it("security: another user's challenge is not found", async () => {
    ...
    const aToken = (await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword, changeScope)).data
      .access_token;
    const started = await startChange(ctx, aToken, `a-new-${Date.now()}@me-email.example.com`);
    ...
    const bVerify = await submitChangeCode(ctx, bToken, started.challengeId, started.code);
    ...
    expect((await submitChangeCode(ctx, aToken, started.challengeId, started.code)).status).toBe(
      200
    );
  });

  it("security: rejects a malformed or non-string new_email before any code is sent", async () => {
    const ctx = await provisionTenant(systemAccessToken, "EMAIL");
    tenants.push(ctx);
    const token = (await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword, changeScope)).data
      .access_token;
    ...
    for (const newEmail of [
      ...
      console.log("malformed new_email:", JSON.stringify(newEmail), resp.status);
      expect(resp.status).toBe(400);
  ...
  it("code robustness: wrong code, reuse, and retry limit", async () => {
    ...
    const token = (await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword, changeScope)).data
      .access_token;
    const started = await startChange(ctx, token, `robust-${Date.now()}@me-email.example.com`);

    const wrong = await submitChangeCode(ctx, token, started.challengeId, "000000");
    expect(wrong.status).toBe(400);
    expect((await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword)).status).toBe(200);

    // The correct code still works after a failed attempt.
    expect((await submitChangeCode(ctx, token, started.challengeId, started.code)).status).toBe(200);

    // A consumed challenge is gone.
    const reused = await submitChangeCode(ctx, token, started.challengeId, started.code);
  ...
  it("EMAIL policy: rejects changing to an email already used by another user", async () => {
    ...
    const token = (await passwordGrant(ctx, ctx.adminEmail, ctx.adminPassword, changeScope)).data
      .access_token;
    const resp = await runEmailChange(ctx, token, otherEmail);
  ...
  it("USERNAME policy: allows changing to a duplicate email (email is not the identifier)", async () => {
    ...
    const token = (await passwordGrant(ctx, ctx.adminName, ctx.adminPassword, changeScope)).data
      .access_token;
    const resp = await runEmailChange(ctx, token, sharedEmail);
  ...
  it("audit: emits verify / change events separately", async () => {
    ...
    expect((await runEmailChange(ctx, token, `audit-${Date.now()}@me-email.example.com`)).status)
      .toBe(200);
