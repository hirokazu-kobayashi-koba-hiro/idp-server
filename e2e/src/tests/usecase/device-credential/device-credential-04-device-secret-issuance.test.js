...
describe("Device Credential Use Case: Device Secret Issuance via FIDO-UAF Registration", () => {
  ...
  /**
   * Parameterized test for device secret length validation per OIDC Core Section 16.19
   * - HS256: at least 32 bytes (256 bits)
   * - HS384: at least 48 bytes (384 bits)
   * - HS512: at least 64 bytes (512 bits)
   */
  it.each([
    { algorithm: "HS256", minBytes: 32 },
    { algorithm: "HS384", minBytes: 48 },
    { algorithm: "HS512", minBytes: 64 },
  ])("should issue device secret with correct length for $algorithm (min $minBytes bytes)", async ({ algorithm, minBytes }) => {
    const timestamp = Date.now();
    const organizationId = uuidv4();
    const tenantId = uuidv4();
    const clientId = uuidv4();
    ...
    console.log(`\n=== Device Secret Length Validation: ${algorithm} (min ${minBytes} bytes) ===\n`);

    const { jwks } = await generateRS256KeyPair();

    // Create tenant with specified algorithm
    const onboardingRequest = {
      organization: {
        id: organizationId,
        name: `Device Secret ${algorithm} Test Org ${timestamp}`,
        description: `Test organization for ${algorithm} device secret`,
      },
      tenant: {
        id: tenantId,
        ...
        identity_policy_config: {
          ...
          authentication_device_rule: {
            ...
            authentication_type: "device_secret_jwt",
            issue_device_secret: true,
            device_secret_algorithm: algorithm, // Parameterized algorithm
      ...
      authorization_server: {
        ...
        jwks_uri: `${backendUrl}/${tenantId}/v1/jwks`,
        jwks: jwks,
        grant_types_supported: ["authorization_code", "password", "urn:ietf:params:oauth:grant-type:jwt-bearer"],
        token_signed_key_id: "signing_key_1",
        id_token_signed_key_id: "signing_key_1",
    ...
    await postWithJson({
      ...
      body: {
        ...
        interactions: {
          "password-authentication": {
            request: { schema: { type: "object", properties: { username: { type: "string" }, password: { type: "string" } } } },
            pre_hook: {},
            execution: { function: "password_verification" },
            post_hook: {},
            response: {},
    ...
    // Create test client with JWT Bearer Grant
    await postWithJson({
      url: `${backendUrl}/v1/management/organizations/${organizationId}/tenants/${tenantId}/clients`,
      headers: { Authorization: `Bearer ${adminAccessToken}` },
      body: {
        client_id: clientId,
        client_secret: clientSecret,
        client_name: `Device Secret ${algorithm} Test Client`,
        redirect_uris: [redirectUri],
    ...
    const challengeResponse = await postWithJson({
      url: `${backendUrl}/${tenantId}/v1/authorizations/${authId}/fido-uaf-registration-challenge`,
      body: { app_name: "Test App", platform: "iOS", os: "iOS 17", model: "iPhone 15" },
    });
    expect(challengeResponse.status).toBe(200);

    const registrationCompleteResponse = await postWithJson({
      url: `${backendUrl}/${tenantId}/v1/authorizations/${authId}/fido-uaf-registration`,
      body: { uafResponse: challengeResponse.data.uafRequest ? [{ assertionScheme: "UAFV1TLV", assertion: "mock_assertion_data" }] : [] },
    });
    expect(registrationCompleteResponse.status).toBe(200);
    ...
    expect(deviceSecretAlgorithm).toBe(algorithm);
    console.log(`✅ Device secret issued with algorithm: ${deviceSecretAlgorithm}`);

    // CRITICAL: Verify secret byte length per OIDC Core Section 16.19
    const secretBytes = Buffer.from(deviceSecret, "utf8");
    console.log(`   Secret length: ${secretBytes.length} bytes (required: ${minBytes} bytes)`);

    expect(secretBytes.length).toBeGreaterThanOrEqual(minBytes);
    console.log(`✅ Secret length validation PASSED: ${secretBytes.length} >= ${minBytes} bytes`);
    ...
    // Test JWT Bearer Grant with the issued secret
    const issuer = `${backendUrl}/${tenantId}`;
    const assertionPayload = {
      iss: deviceSecretJwtIssuer,
      sub: userId,
    ...
    console.log(`JWT Bearer Grant (${algorithm}) response:`, jwtBearerResponse.status);
    if (jwtBearerResponse.status !== 200) {
      console.log("JWT Bearer Grant error:", JSON.stringify(jwtBearerResponse.data, null, 2));
    }
    expect(jwtBearerResponse.status).toBe(200);
    console.log(`✅ JWT Bearer Grant with ${algorithm} succeeded\n`);
    ...
    console.log(`✅ Device secret issued with ${algorithm}`);
    console.log(`✅ Secret length: ${secretBytes.length} bytes (min: ${minBytes})`);
    console.log(`✅ JWT Bearer Grant authentication succeeded`);
  });
